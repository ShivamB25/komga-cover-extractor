...
try:
    ...
    print(
        "WARN: Could not import from .config in metadata_utils.py, using placeholders."
    )
...
try:
    ...
    from .archive_utils import (
        contains_comic_info,
        get_file_from_zip,
    )  # Use functions from archive_utils
    ...
    def contains_comic_info(*args):
        return False

    def get_file_from_zip(*args, **kwargs):
        return None
...
def parse_comicinfo_xml(xml_string):
    ...
    try:
        ...
        send_message(
            f"Failed to parse ComicInfo XML: {e}", error=True
        )  # Use imported log_utils function
    except Exception as e:
        send_message(
            f"Unexpected error parsing ComicInfo XML: {e}", error=True
        )  # Use imported log_utils function
    return tags
...
def parse_html_tags(html_content):
    ...
    try:
        ...
        send_message(
            f"Error parsing HTML/OPF content: {e}", error=True
        )  # Use imported log_utils function
    return tags
    ...
    try:
        if extension in manga_extensions:  # Use imported config value
            if contains_comic_info(file_path):  # Use imported archive_utils function
                comicinfo_data = (
                    get_file_from_zip(  # Use imported archive_utils function
            ...
            opf_searches = [
                ...
                r"volume\.opf$",
                r"metadata\.opf$",
                r"\.opf$",  # General fallback
            ]
            # Use get_file_from_zip to find the OPF file content
            opf_content = get_file_from_zip(
                file_path, opf_searches, ".opf", allow_base=False
            )  # Use imported archive_utils function
            if opf_content:
        ...
        send_message(
            f"Failed to retrieve metadata from {file_path}: {e}", error=True
        )  # Use imported log_utils function
    return metadata
...
def get_novel_cover(novel_path):
    ...
    try:
        with zipfile.ZipFile(novel_path) as z:
            ...
            try:
                ...
                rootfile_path = container_tree.xpath(
                    "/u:container/u:rootfiles/u:rootfile/@full-path",
                ...
                send_message(
                    f"META-INF/container.xml not found in {novel_path}", error=True
                )
                return None
            ...
            try:
                ...
                send_message(
                    f"Root file '{rootfile_path}' not found in {novel_path}", error=True
                )
                return None
            except etree.XMLSyntaxError as e:
                send_message(
                    f"Error parsing OPF file '{rootfile_path}' in {novel_path}: {e}",
                    error=True,
                )
                return None
            ...
            cover_id = opf_tree.xpath(
                "//opf:metadata/opf:meta[@name='cover']/@content", namespaces=namespaces
            )
            if not cover_id:
            ...
            if cover_id:
                ...
                cover_href = opf_tree.xpath(
                    f"//opf:manifest/opf:item[@id='{cover_id}']/@href",
                    namespaces=namespaces,
                )
                if cover_href:
                    ...
                    cover_path = os.path.join(
                        os.path.dirname(rootfile_path), cover_href_decoded
                    )
        ...
        send_message(
            f"Bad zip file: {novel_path}", error=True
        )  # Use imported log_utils function
    except Exception as e:
        send_message(
            f"Error reading EPUB {novel_path}: {e}", error=True
        )  # Use imported log_utils function
    return None
                ...
                for name in zf.namelist():
                    ...
                    try:
                        with zf.open(name) as file_content_stream:
                            ...
                            content_sample = file_content_stream.read(2048).decode(
                                "utf-8", errors="ignore"
                            )
                        ...
                        send_message(
                            f"Error reading {name} in {file_path}: {read_err}",
                            error=True,
                        )  # Use imported log_utils function

    except zipfile.BadZipFile:
        send_message(
            f"Bad zip file, cannot check premium status: {file_path}", error=True
        )  # Use imported log_utils function
    except Exception as e:
        send_message(
            f"Error checking premium status for {file_path}: {e}", error=True
        )  # Use imported log_utils function
