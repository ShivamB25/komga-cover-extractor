...
def execute_command(command):
    ...
    try:
        ...
        send_message(f"Error executing command '{command}': {e}", error=True) # Use imported log_utils function
...
def get_input_from_user(
    ...
    def input_with_timeout(prompt_str, shared_variable_dict):
        try:
            ...
            if not acceptable_values or user_input in acceptable_values:
             ...
             shared_variable_dict["done"] = True # Signal completion regardless of input validity

    # Format the prompt
    if example:
        example_str = f" or ".join(map(str, example)) if isinstance(example, list) else str(example)
    ...
    timer = None
    input_thread = threading.Thread(target=input_with_timeout, args=(prompt_str, shared_variable))
    ...
    if timer: timer.cancel() # Cancel timer if input was received before timeout
...
def get_session_object(url):
    ...
    try:
        ...
        send_message(f"Error getting session object for {url}: {e}", error=True) # Use imported log_utils function
        return None
...
def scrape_url(url, strainer=None, headers=None, cookies=None, proxy=None):
    ...
    try:
        ...
        if not session_object: return None
        ...
        send_message(f"Error scraping URL {url}: {e}", error=True) # Use imported log_utils function
        return None
    ...
    try:
        if isinstance(volume_number, list):
            ...
            if len(processed_nums) == 2:
             ...
             return int(volume_number) if float(volume_number).is_integer() else float(volume_number)
    except (ValueError, TypeError) as e:
        if not silent:
            send_message(f"Failed to convert number: {volume_number}\nERROR: {e}", error=True) # Use imported log_utils function
        return ""
...
def is_same_index_number(index_one, index_two, allow_array_match=False):
    ...
    if (num_one == num_two and num_one != ""):
         ...
         list_one = index_one if isinstance(index_one, (list, tuple)) else [num_one] if num_one != "" else []
         list_two = index_two if isinstance(index_two, (list, tuple)) else [num_two] if num_two != "" else []

         # Check for overlap
         if list_one and list_two:
             set_one = set(set_num_as_float_or_int(i, silent=True) for i in list_one if set_num_as_float_or_int(i, silent=True) != "")
             set_two = set(set_num_as_float_or_int(i, silent=True) for i in list_two if set_num_as_float_or_int(i, silent=True) != "")
