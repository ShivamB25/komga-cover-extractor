...
from .config import (
    watchdog_file_transferred_check_interval, download_folders, paths,
    file_extensions, image_extensions, compress_image_option, ignored_folder_names,
    check_for_existing_series_toggle, cached_paths, series_cover_file_names,
    use_latest_volume_cover_as_series_cover,
    manual_rename, mute_discord_rename_notifications, watchdog_toggle,
    transferred_files, transferred_dirs, rar_extensions, chapter_support_toggle,
...
from .string_utils import ( # Import directly
    is_volume_one, filter_non_chapters, clean_str, remove_brackets,
    contains_volume_keywords, contains_chapter_keywords # Needed by placeholders
)
...
def set_modification_date(file_path, date):
    ...
    try:
        ...
        access_time = get_modification_date(file_path) # Keep access time same as current mod time
...
def is_file_transferred(file_path):
    ...
    try:
        ...
        time.sleep(watchdog_file_transferred_check_interval) # Use imported config value
...
def get_header_extension(file):
    ...
    if extension_from_name in manga_extensions + rar_extensions: # Use imported config values
        try:
...
def remove_ignored_folders(dirs):
    ...
    return [x for x in dirs if x not in ignored_folder_names] # Use imported config value
...
def clean_and_sort(
    ...
    if (
        ...
        and not test_mode
        and root not in cached_paths + download_folders + paths # Use imported config values
    ...
    if ignored_folder_names and not skip_remove_ignored_folders: # Use imported config value
        ignored_parts = any(
    ...
    if files:
        ...
        if not skip_remove_unaccepted_file_types and files:
            ...
            files = remove_unaccepted_file_types(files, root, accepted_ext, test_mode=test_mode) # Use local function
        if just_these_files and files:
            files = [
                ...
                or (
                    ...
                    and get_file_extension(x) in image_extensions # Use imported config value
                )
            ]
...
def process_files_and_folders(
    ...
    effective_keep_images = keep_images_in_just_these_files if in_download_folders else False
...
def get_all_folders_recursively_in_dir(dir_path):
    ...
    try:
        for root, dirs, files in scandir.walk(dir_path):
            ...
            dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ignored_folder_names] # Use imported config value
...
def get_all_files_in_directory(dir_path):
    ...
    try:
        for root, dirs, files in scandir.walk(dir_path):
            ...
            cleaned_files, _ = clean_and_sort(root, files=files, dirs=dirs) # Use local function
        ...
        for root, dirs, files in scandir.walk(dir_path):
            ...
            for file in files:
                ...
                if file_path not in results:
                    ...
                    is_image = extension in image_extensions # Use imported config value
                    # Include non-images, or images only if compression is off AND it's an existing library path
                    is_existing_lib_path = any(dir_path.startswith(p) for p in paths) # Use imported config value
                    if not is_image or (not compress_image_option and is_existing_lib_path): # Use imported config value
                        results.append(file_path)
    except Exception as e:
        send_message(f"Error walking directory {dir_path} for watchdog: {e}", error=True)
    return results
...
def remove_file(full_file_path, silent=False):
    ...
    if not os.path.isfile(full_file_path):
        if not silent: send_message(f"{full_file_path} is not a file or does not exist.", error=True)
        return False
    try:
        os.remove(full_file_path)
        if os.path.isfile(full_file_path): # Check if removal failed
            if not silent: send_message(f"Failed to remove {full_file_path}.", error=True) # Use imported log_utils function
            return False
        if not silent:
            send_message(f"File removed: {full_file_path}", discord=False) # Use imported log_utils function
        ...
        if get_file_extension(full_file_path) not in image_extensions: # Use local function / imported config value
            remove_images(full_file_path) # Use local function
        return True
    except OSError as e:
        if not silent: send_message(f"Failed to remove {full_file_path}: {e}", error=True)
        return False
...
def remove_images(path):
    ...
    if series_cover_path:
        ...
        should_remove = False
        if not use_latest_volume_cover_as_series_cover and is_vol_one: # Use imported config value
            should_remove = True
        elif use_latest_volume_cover_as_series_cover and os.path.isfile(volume_cover_for_series):
             # Simplified check: remove if modification times or hashes match
             try:
                 if get_modification_date(series_cover_path) == get_modification_date(volume_cover_for_series) or \
                    get_file_hash(series_cover_path) == get_file_hash(volume_cover_for_series): # Use local functions
                     should_remove = True
             except Exception as e:
                 send_message(f"Error comparing series/volume covers for removal: {e}", error=True)
...
def check_and_delete_empty_folder(folder):
    ...
    try:
        ...
        folder_contents = [item for item in os.listdir(folder) if not item.startswith('.')]
        ...
        if len(folder_contents) == 1:
            ...
            if base_name in series_cover_file_names and ext in image_extensions: # Use imported config values
        ...
        if not folder_contents and folder not in paths + download_folders: # Use imported config values
            try:
                os.rmdir(folder)
        ...
        if os.path.isfile(image_path):
            ...
            try:
                ...
                shutil.move(image_path, dest_image_path)
                 ...
                 send_message(f"Error moving volume cover {image_path} to {folder_name}: {e}", error=True)

    # Move series cover (e.g., cover.jpg)
    for cover_base_name in series_cover_file_names: # Use imported config value
        for extension in image_extensions:
            series_cover_src = os.path.join(file_obj.root, f"{cover_base_name}{extension}")
            if os.path.isfile(series_cover_src):
                series_cover_dest = os.path.join(folder_name, f"{cover_base_name}{extension}")
                try:
                    # Logic for deciding whether to overwrite or keep existing series cover
                    should_move = True
                    if os.path.isfile(series_cover_dest):
                        is_vol_one = is_volume_one(file_obj.name) # Use local placeholder
                        is_highest = is_same_index_number(file_obj.index_number, highest_index_num, allow_array_match=True) if hasattr(file_obj, 'index_number') else False # Use local placeholder

                        if use_latest_volume_cover_as_series_cover and hasattr(file_obj, 'file_type') and file_obj.file_type == "volume" and is_highest: # Use imported config value
                            # Compare hashes/mod times before overwriting
                            src_mod_time = get_modification_date(series_cover_src) # Use local function
                            ...
                            if src_mod_time == dest_mod_time:
                                ...
                                src_hash = get_file_hash(series_cover_src) # Use local function
                                dest_hash = get_file_hash(series_cover_dest)
                                if src_hash == dest_hash:
                                    # Update mod time of dest if hashes match but times differ
                                    set_modification_date(series_cover_dest, src_mod_time) # Use local function
                                    should_move = False # Keep existing
                                else:
                                    remove_file(series_cover_dest, silent=True) # Remove old if hashes differ
                        elif not use_latest_volume_cover_as_series_cover and is_vol_one:
                             remove_file(series_cover_dest, silent=True) # Overwrite if using vol 1 logic
                        else:
                            should_move = False # Don't move if conditions aren't met

                    if should_move:
                        shutil.move(series_cover_src, series_cover_dest)
                    elif os.path.isfile(series_cover_src): # If we decided not to move, remove the source
                         remove_file(series_cover_src, silent=True)

                except Exception as e:
                    send_message(f"Error moving series cover {series_cover_src} to {folder_name}: {e}", error=True)
...
def remove_folder(folder):
    ...
    result = False
    if os.path.isdir(folder) and (folder not in download_folders + paths): # Use imported config values
        try:
        ...
        try:
            if not os.path.isdir(new_folder_path):
                ...
                if os.path.isdir(new_folder_path):
                    result = True
                    if not silent:
                        send_message(f"\n\t\tMoved Folder: {folder_name} from {os.path.dirname(folder)} to {new_location}", discord=False) # Use imported log_utils function
                        # Discord notification responsibility moved to calling function or discord_utils
            else:
                 send_message(f"\t\tFolder already exists: {new_folder_path}", error=True) # Use imported log_utils function
        except Exception as e:
            send_message(f"\t\tFailed to move folder: {folder_name} to {new_location} - {e}", error=True)
    return result
        ...
        try:
            ...
            if not os.path.isfile(new_file_path): # Avoid overwriting existing file
                ...
                if os.path.isfile(new_file_path):
                    if not silent:
                        send_message(f"\t\tMoved File: {file_obj.name} to {new_location}", discord=False) # Use imported log_utils function
                        # Discord notification responsibility moved to calling function or discord_utils
                    # Update file object path after move before moving images
                    original_path = file_obj.path
                    file_obj.path = new_file_path
                    file_obj.extensionless_path = get_extensionless_name(new_file_path) # Use local function
                    file_obj.root = new_location
                    move_images(file_obj, new_location, highest_index_num, is_chapter_dir) # Use local function
                    return True
                else:
                    send_message(f"\t\tMove verification failed for: {new_file_path}", error=True)
                 ...
                 send_message(f"\t\tFile already exists at destination: {new_file_path}, skipping move.", error=True)
                 return False
        except Exception as e:
            send_message(f"Error moving file {file_obj.name} to {new_location}: {e}", error=True)
            return False
    return False
...
def replace_file(old_file, new_file, highest_index_num=""):
    ...
    if os.path.isfile(old_file.path) and os.path.isfile(new_file.path):
        if remove_file(old_file.path): # Use local function
            if move_file(new_file, old_file.root, silent=True, highest_index_num=highest_index_num): # Use local function
                result = True
                ...
                send_message(f"\t\tReplaced: {old_file.name} with {new_file.name} in {old_file.root}", discord=False) # Use imported log_utils function
                # Discord notification responsibility moved to calling function or discord_utils
            else:
                send_message(f"\tFailed to move new file: {new_file.name} after removing old file.", error=True) # Use imported log_utils function
        else:
            send_message(f"\tFailed to remove old file: {old_file.name}. Replacement aborted.", error=True)
    else:
        send_message(f"\tOne or both files missing for replacement: {old_file.name}, {new_file.name}", error=True)
    return result

def normalize_path(path):
    """Normalizes path separators and removes Windows drive letters."""
    path = os.path.normpath(path)
    if ":" in path and os.path.isabs(path): # Check if it looks like a Windows absolute path
...
def is_root_present(root_path, target_path):
    ...
    return normalized_target.startswith(normalized_root + "/") or normalized_target == normalized_root
    ...
    for folder in download_folders:
        ...
        for root, dirs, files in scandir.walk(folder):
            ...
            files, dirs = process_files_and_folders( # Use local function
                ...
                is_correct_extensions_feature=file_extensions + rar_extensions # Use imported config values
            )
            ...
            for volume in volumes:
                if not volume.header_extension or volume.extension == volume.header_extension:
                    continue
                ...
                if manual_rename: # Use imported config value
                    user_input = get_input_from_user("\t\t\tRename", ["y", "n"], ["y", "n"]) # Use local placeholder

                if user_input == "y":
                    new_path = f"{volume.extensionless_path}{volume.header_extension}"
                    if rename_file(volume.path, new_path, silent=True): # Use imported file_utils function
                        print("\t\t\tRenamed successfully")
                        if not mute_discord_rename_notifications: # Use imported config value
