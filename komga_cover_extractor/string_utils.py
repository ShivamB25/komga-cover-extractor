...
try:
    from .config import (
        volume_keywords, chapter_keywords, exclusion_keywords, subtitle_exclusion_keywords,
        volume_regex_keywords, exclusion_keywords_joined, subtitle_exclusion_keywords_joined,
    exclusion_keywords_regex, subtitle_exclusion_keywords_regex, chapter_regex_keywords,
    file_extensions_regex, manga_extensions_regex, chapter_searches, chapter_search_patterns_comp,
    ...
    required_similarity_score, publishers, release_groups, exception_keywords,
    zfill_volume_int_value, zfill_volume_float_value, zfill_chapter_int_value, zfill_chapter_float_value,
    preferred_volume_renaming_format, preferred_chapter_renaming_format,
    add_issue_number_to_manga_file_name, manga_extensions, novel_extensions, file_extensions,
    add_publisher_name_to_file_name_when_renaming, search_and_add_premium_to_file_name,
    move_release_group_to_end_of_file_name, release_group_similarity_score,
    replace_unicode_when_restructuring, manual_rename, mute_discord_rename_notifications,
    watchdog_toggle, transferred_files, # Used by reorganize_and_rename
        download_folders, paths # Used by get_series_name_from_volume, is_one_shot
    )
except ImportError:
    ...
    volume_year_regex, publishers_joined, release_groups_joined = "", "", ""
    publishers_joined_regex, release_groups_joined_regex, release_group_end_regex = None, None, None
    required_similarity_score = 0.9
    publishers, release_groups, exception_keywords = [], [], []
    zfill_volume_int_value, zfill_volume_float_value, zfill_chapter_int_value, zfill_chapter_float_value = 2, 4, 3, 5
    preferred_volume_renaming_format, preferred_chapter_renaming_format = "", ""
...
try:
    ...
    from .file_utils import get_file_extension, get_extensionless_name # Removed rename_file, clean_and_sort
    from .misc_utils import set_num_as_float_or_int, isfloat, isint # Removed get_input_from_user
    from .models import Volume, Publisher # Import models needed by functions here
    from .metadata_utils import get_internal_metadata, check_for_premium_content # Import needed metadata functions
    ...
    def set_num_as_float_or_int(n, silent=False): return n
    def isfloat(x): return False
    def isint(x): return False
    class Volume: pass
    class Publisher: pass
    def get_internal_metadata(*args): return None
    def check_for_premium_content(*args): return False
    def get_zip_comment(*args): return ""
...
bracket_avoidance_pattern = re.compile(r"^[\(\[\{].*[\)\]\}]$") # Avoid removing if string IS a bracket
bracket_against_extension_pattern = re.compile(r"(\[[^\]]*?\]|\([^)]*?\)|\{[^}]*?\})(\.\w+)$") # Bracket right before extension
...
def remove_brackets(string):
    ...
    string = re.sub(r"\s*(\[[^\]]*(Digital|Scan|RAW)[^\]]*\]|\([^)]*(Digital|Scan|RAW)[^)]*\))\s*", " ", string, flags=re.IGNORECASE).strip()
    ...
    if ext:
        string_no_ext = get_extensionless_name(string) # Use imported file_utils function
        cleaned_no_ext = bracket_against_extension_pattern.sub("", string_no_ext).strip()
...
def normalize_str(
    ...
    editions = ["Collection", "Master Edition", r"\d-in-\d Edition", "Edition", "Exclusive", "Anniversary", "Deluxe", "Digital", "Official", "Anthology", "Limited", "Complete", "Collector", "Ultimate", "Special"]
    type_keywords = [r"(?<!^)Novel", r"(?<!^)Light Novel", r"(?<!^)Manga", r"(?<!^)Comic", r"(?<!^)LN", r"(?<!^)Series", r"(?<!^)Volume", r"(?<!^)Chapter", r"(?<!^)Book", r"(?<!^)MANHUA"]
    japanese_particles = ["wa", "o", "mo", "ni", "e", "de", "ga", "kara", "to", "ya", r"no(?!\.)", "ne", "yo"]
    misc_words = [r"\d+th", "x", "×", "HD"]
    storefront_keywords = [r"Book\s*walker"]

    if not skip_common_words: words_to_remove.extend(common_words)
    if not skip_editions: words_to_remove.extend(editions)
    if not skip_type_keywords: words_to_remove.extend(type_keywords)
    if not skip_japanese_particles: words_to_remove.extend(japanese_particles)
    if not skip_misc_words: words_to_remove.extend(misc_words)
    if not skip_storefront_keywords: words_to_remove.extend(storefront_keywords)
...
def clean_str(
    ...
    s = s.replace(":", " - ") if not skip_colon_replace and ":" in s else s # Replace colon with hyphen-space
    s = remove_dual_space(s) # Use local function
    s = remove_brackets(s) if not skip_bracket and contains_brackets(s) else s # Use local functions
    s = unidecode(s) if not skip_unidecode and contains_unicode(s) else s # Use local function
    s = normalize_str(s) if not skip_normalize else s # Use local function
    s = remove_punctuation(s) if not skip_punctuation and contains_punctuation(s) else s # Use local functions
    # s = remove_s(s) if not skip_remove_s else s # Removing 's' can be problematic, disable by default
    s = remove_dual_space(s) # Use local function
    s = convert_to_ascii(s) if not skip_convert_to_ascii and contains_unicode(s) else s # Use local functions
    s = replace_underscores(s) if not skip_underscore and "_" in s else s # Use local function
...
volume_regex = re.compile(
    ...
    r"\b(%s)\s*([0-9]+(?:[._-][0-9]+)?)\b" % volume_regex_keywords, # Use imported config value
    re.IGNORECASE,
)
...
def contains_volume_keywords(file):
    ...
    clean_file = replace_underscores(file).strip() if "_" in file else file.strip() # Use local function
...
def contains_chapter_keywords(file_name):
    ...
    file_name_clean = replace_underscores(file_name_clean).strip() if "_" in file_name_clean else file_name_clean # Use local function
    ...
    for pattern in chapter_search_patterns_comp: # Use imported config value
        ...
        if result:
            ...
            if not (starts_with_bracket(result_group) and ends_with_bracket(result_group) and re.search(r"^\([({[]\d{4}[\])}]\)$", result_group)): # Use local functions
                found = True
                break
    ...
    if not found and not contains_volume_keywords(file_name): # Use local function
        without_year = re.sub(volume_year_regex, "", file_name, flags=re.IGNORECASE) # Use imported config value
        without_year = re.sub(r"\b(19|20)\d{2}\b", "", without_year).strip() # Remove potential years more broadly
        # Look for numbers not preceded by volume keywords
        # This regex needs careful testing to avoid false positives
        lone_number_match = re.search(r"(?<!(%s)\s*)\b\d+([._]\d+)?\b" % volume_regex_keywords, without_year, re.IGNORECASE) # Use imported config value
...
def contains_keyword(file_string, chapter=False):
    ...
    keywords = chapter_regex_keywords if chapter else volume_regex_keywords # Use imported config values
...
def check_for_exception_keywords(file_name, exception_keywords_list):
    ...
    if not exception_keywords_list: return False
...
def get_release_number(file, chapter=False):
    ...
    def clean_series_name(name):
        ...
        if "." in name: name = re.sub(r"^\s*(\.)", "", name, re.IGNORECASE).strip()
        # Remove subtitle
        ...
        if "#" in name:
            ...
            if re.search(r"(\d+#\d+)", name): name = re.sub(r"((#)([0-9]+)(([-_.])([0-9]+)|)+)", "", name).strip()
        # removes part like x1
        if "x" in name: name = re.sub(r"(x[0-9]+)", "", name, re.IGNORECASE).strip()
        # removes brackets
        if contains_brackets(name): name = remove_brackets(name).strip() # Use outer scope function
        # Removes extension and optional preceding text
        name = re.sub(r"(((\s+)?-(\s+)?([A-Za-z]+))?(%s))" % file_extensions_regex, "", name, re.IGNORECASE).strip() # Use outer scope config
    ...
    keywords = volume_regex_keywords if not chapter else chapter_regex_keywords # Use outer scope config
    result = None

    file = remove_dual_space(file.replace("_extra", ".5")).strip() # Use outer scope function
    file = replace_underscores(file) if "_" in file else file # Use outer scope function

    # Check for multi-volume pattern first
    is_multi_volume = check_for_multi_volume_file(file, chapter=chapter) if "-" in file else False # Use outer scope function

    if not chapter:
        # Search for volume number pattern
        result = volume_number_search_pattern.search(file) # Use outer scope compiled pattern
    else:
        # Chapter search logic
        if has_multiple_numbers(file): # Use outer scope function
            extension_less_file = get_extensionless_name(file) # Use imported file_utils function
            if chapter_number_search_pattern.search(extension_less_file): # Use outer scope compiled pattern
                file_part = chapter_number_search_pattern.sub("", extension_less_file)
                if file_part.endswith("-") and not re.search(r"-(\s+)?(#)?([0-9]+)(([-_.])([0-9]+)|)+(x[0-9]+)?(\s+)?-", file_part):
                    file = file_part[:-1].strip() # Update file for subsequent search if pattern removed suffix

        # Find the first matching chapter pattern
        result = next((r for pattern in chapter_search_patterns_comp if (r := pattern.search(file))), None) # Use outer scope config
    ...
    if result:
        try:
            ...
            if contains_non_numeric(file_match_str): # Use outer scope function
                file_match_str = re.sub(r"\b({})(\.|)([-_. ])?".format(keywords), "", file_match_str, flags=re.IGNORECASE).strip()
                # Handle cases like 1v1
                if contains_non_numeric(file_match_str) and re.search(r"\b[0-9]+({})[0-9]+\b".format(keywords), file_match_str, re.IGNORECASE):
                    file_match_str = re.sub(r"({})".format(keywords), ".", file_match_str, flags=re.IGNORECASE).strip()

            # Extract numbers
            if is_multi_volume or (("-" in file_match_str or "_" in file_match_str) and re.search(r"([0-9]+(\.[0-9]+)?)([-_]([0-9]+(\.[0-9]+)?))+", file_match_str)):
                if not is_multi_volume: is_multi_volume = True
                multi_numbers = get_min_and_max_numbers(file_match_str) # Use outer scope function
                if multi_numbers:
                    results.extend(set_num_as_float_or_int(num, silent=True) for num in multi_numbers) # Use imported misc_utils function
                ...
                if num_str.endswith("0") and ".0" in num_str: num_str = num_str.split(".0")[0] # Remove trailing .0
                results = set_num_as_float_or_int(num_str, silent=True) # Use imported misc_utils function

        except (ValueError, TypeError, AttributeError) as e:
            send_message(f"Error parsing release number from '{result.group() if hasattr(result, 'group') else file}': {e}", error=True) # Use imported log_utils function
    ...
    if results or results == 0:
        if is_multi_volume and isinstance(results, list) and len(results) > 1:
            ...
            return results
        elif isinstance(results, list) and len(results) == 1: # Single item list from multi-vol parse
             num = results[0]
             if not chapter and isinstance(num, (int, float)) and num >= 2000: return ""
             return num
...
def get_file_part(file, chapter=False, series_name=None, subtitle=None):
    ...
    rx_remove_vol_num = re.compile(
        r".*(%s)([-_. ]|)([-_. ]|)([0-9]+)(\b|\s)" % volume_regex_keywords, # Use imported config value
        re.IGNORECASE,
    )
    ...
    contains_keyword = re.search(r"\bpart\b", file, re.IGNORECASE) if "part" in file.lower() else ""
    ...
    if not chapter:
        ...
        if contains_indicator:
            ...
            if search:
                part_search = re.search(r"((x|#)([0-9]+)(([-_.])([0-9]+)|)+)", search.group(), re.IGNORECASE)
                if part_search:
                    # remove the x or # prefix
                    result = rx_remove_x_hash_num.sub("", part_search.group())

    # Convert the extracted number string to int/float
    result = set_num_as_float_or_int(result, silent=True) # Use imported misc_utils function
...
def get_min_and_max_numbers(string):
    ...
    numbers_search = [set_num_as_float_or_int(num, silent=True) for num in numbers_search if num]
    ...
    if numbers_search:
        try:
            ...
            if highest_number is not None and highest_number != lowest_number:
             ...
             send_message(f"Error finding min/max in get_min_and_max_numbers for '{string}': {e}", error=True) # Use imported log_utils function
...
def has_one_set_of_numbers(string, chapter=False, file=None, subtitle=None):
    ...
    return len(extract_all_numbers(string, subtitle=subtitle)) == 1 # Placeholder using another function
...
def extract_all_numbers(string, subtitle=None):
    ...
    if subtitle: string = remove_dual_space(re.sub(rf"(-|:)\s*{re.escape(subtitle)}", "", string, re.IGNORECASE).strip()) # Use local function
    string = replace_underscores(string) if "_" in string else string # Use local function
    numbers_found = re.findall(r'\b\d+(?:[._]\d+)?\b', string)
    processed_numbers = []
    for num_str in numbers_found:
         num = set_num_as_float_or_int(num_str, silent=True) # Use imported misc_utils function
         if num != "": processed_numbers.append(num)
...
def get_series_name_from_volume(name, root, test_mode=False, second=False):
    ...
    cleaned_name = re.sub(r"(%s)$" % file_extensions_regex, "", cleaned_name).strip() # Use imported config value
    cleaned_name = remove_brackets(cleaned_name) # Use local function
    return cleaned_name.strip()

@lru_cache(maxsize=3500)
def chapter_file_name_cleaning(file_name, chapter_number="", skip=False, regex_matched=False):
...
def get_series_name_from_chapter(name, root, chapter_number="", second=False):
    ...
    name = re.sub(r"\s+c(h(ap)?)?(\.)?\s*\d+([._]\d+)?\s*$", "", name, flags=re.IGNORECASE).strip()
    name = re.sub(r"\s+\d+([._]\d+)?\s*$", "", name).strip() # Remove trailing number
    return remove_brackets(name).strip() # Use local function

def get_series_name_from_contents(folder_name, file_names, required_matching_percent=100):
    """Derives series name by finding common prefix between folder and filenames."""
    # TODO: Implement full logic
    return folder_name # Placeholder

@lru_cache(maxsize=3500)
def get_extra_from_group(name, groups, publisher_m=False, release_group_m=False, series_name=None):
...
def get_extras(file_name, chapter=False, series_name="", subtitle=""):
    ...
    extras = [b for b in all_brackets if not re.match(r"^\(?\d{4}\)?$", b.strip("()[]{} "))]
...
def get_subtitle_from_title(file, publisher=None): # Expects Volume object
    ...
    try:
        if file.series_name: # Ensure series_name exists
         ...
         send_message(f"Regex error escaping series name '{file.series_name}': {escape_error}", error=True) # Use imported log_utils
         without_series_name = file.name # Fallback to full name

    # First Search: Look for content after ' - ' or ': '
    dash_or_colon_search = get_subtitle_from_dash(without_series_name) # Use local function
    ...
    if publisher and (publisher.from_meta or publisher.from_name) and not year_or_digital_search:
        pub_pattern = ""
        if publisher.from_meta:
            try:
                pub_pattern = re.escape(publisher.from_meta)
                publisher_search = re.search(rf"[\[\{{\(]]({pub_pattern})[\]\}}\)]", without_series_name, re.IGNORECASE)
            except re.error: publisher_search = None # Handle regex error
    ...
    if dash_or_colon_search and (year_or_digital_search or publisher_search):
        ...
        if file.extension and subtitle.endswith(file.extension): # Check if extension exists
            subtitle = get_extensionless_name(subtitle) # Use imported file_utils function

        # Remove trailing metadata (year/digital or publisher)
        if publisher_search:
             # Remove from the publisher bracket onwards
             try:
                 pub_pattern_found = publisher_search.group(1) # The actual matched publisher name
                 # Ensure pattern exists before trying to remove
                 if pub_pattern_found:
                      subtitle = re.sub(rf"\s*[\[\{{\(]{re.escape(pub_pattern_found)}[\]\}}\)].*", "", subtitle, flags=re.IGNORECASE).strip()
             except (re.error, AttributeError): pass # Ignore regex error or if group doesn't exist
        elif year_or_digital_search:
            # Remove from the year/digital bracket onwards
            subtitle = re.sub(r"\s*[\[\{\(](?:\d{4}|Digital)[\]\}\)].*", "", subtitle, flags=re.IGNORECASE).strip()
        ...
        try:
            ...
            if subtitle and folder_basename and re.search(rf"\b{re.escape(subtitle)}\b", folder_basename, re.IGNORECASE):
                 subtitle = "" # Likely part of series name if in folder name
        except Exception: pass # Ignore errors in path manipulation
        ...
        if file.volume_number and subtitle:
             ...
             if volume_regex_keywords and re.search(rf"^({volume_regex_keywords})(\s+)?(0+)?{re.escape(vol_num_str)}$", subtitle.strip(), re.IGNORECASE): # Use imported config value
                 subtitle = ""
...
def get_release_year(name, metadata=None):
    ...
    match = re.search(volume_year_regex, name, re.IGNORECASE) # Use imported config value
    if match:
...
def get_publisher_from_meta(metadata):
    ...
    publisher = None
    if metadata:
        if "Publisher" in metadata: publisher = metadata["Publisher"]
        elif "dc:publisher" in metadata: publisher = metadata["dc:publisher"]
    if publisher:
...
def get_identifiers(zip_comment):
    ...
    if zip_comment and "Identifiers:" in zip_comment:
        try:
            ...
            metadata = [id_str.strip() for id_str in identifiers_part.split(",") if id_str.strip()]
        except IndexError:
...
def get_series_name(dir_name):
    ...
    dir_name = remove_dual_space(dir_name.replace("_extra", ".5")).strip() # Use local function
    dir_name = re.sub(r"(\b|\s)((\s|)-(\s|)|)(Part|)(%s)([-_. ]|)([-_. ]|)([0-9]+)(\b|\s).*$" % volume_regex_keywords, "", dir_name, flags=re.IGNORECASE).strip() # Use imported config value
...
def parse_words(user_string):
    ...
    if user_string:
        try:
            ...
            try:
                words_no_uni = unidecode(words_lower) if contains_unicode(words_lower) else words_lower # Use local function
            except Exception:
                words_no_uni = words_lower # Fallback if unidecode fails
            words = words_no_uni.split()
        except Exception as e:
            send_message(f"parse_words(string='{user_string}') - Error: {e}", error=True) # Use imported log_utils function
    return words
...
def find_consecutive_items(arr1, arr2, count=3):
    ...
    tup1 = tuple(arr1)
    tup2 = tuple(arr2)
    if len(tup1) < count or len(tup2) < count: return False
...
def move_strings_to_top(target_item, item_array):
    ...
    if not target_words: return item_array
    ...
    for item in item_array:
        try:
            ...
            if item_prefix == target_prefix and item not in moved_set:
             ...
             send_message(f"Error processing item '{item}' in move_strings_to_top: {e}", error=True) # Use imported log_utils function
             if item not in moved_set: # Ensure item is added to remaining if error occurs
                 remaining_items.append(item)


    if not items_to_move: return item_array # Return original if nothing matched

    # Simple sort: exact match first, then others
    clean_target = clean_str(target_item) # Use local function
    items_to_move.sort(key=lambda x: clean_str(os.path.basename(x)) != clean_target) # Use local function
