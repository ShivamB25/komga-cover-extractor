...
from .log_utils import (
    get_lines_from_file,
    send_message,
)  # Assuming get_lines_from_file is moved or available
...
chapter_searches = [
    ...
    % (
        ...
        chapter_regex_keywords,
        "|".join(manga_extensions).replace(".", r"\."),
    ...
    % (
        chapter_regex_keywords,
        exclusion_keywords_regex,
        "|".join(file_extensions).replace(".", r"\."),
...
library_types = [
    LibraryType(
        ...
        manga_extensions,
        [r"\(Digital\)"],
        [r"Webtoon", r"^(?=.*Digital)((?=.*Compilation)|(?=.*danke-repack))"],
        1,
    ),
    LibraryType(
        "light novel",
...
def load_list_from_file(filename, target_list):
    ...
    if os.path.isfile(filepath):
        try:
            ...
            send_message(
                f"Error loading {filename}: {e}", error=True
...
release_groups_joined = (
    "|".join(map(re.escape, release_groups)) if release_groups else ""
)

publishers_joined_regex = (
    re.compile(rf"(?<=[\(\[\{{])({publishers_joined})(?=[\)\]\}}])", re.IGNORECASE)
    if publishers_joined
    else None
)
release_groups_joined_regex = (
    re.compile(rf"(?<=[\(\[\{{])({release_groups_joined})(?=[\)\]\}}])", re.IGNORECASE)
    if release_groups_joined
    else None
)
# Construct the regex string first, then compile
# Construct the regex string first using an f-string, escaping regex braces
_rg_end_pattern_fstring = (
    f"-(?! )([^\\(\\)\\[\\]\\{{}}\\+]+)(?:{file_extensions_regex})$"
    if file_extensions_regex
    else None
)
# Now compile the resulting string
release_group_end_regex = (
    re.compile(_rg_end_pattern_fstring, re.IGNORECASE)
    if _rg_end_pattern_fstring
    else None
)
