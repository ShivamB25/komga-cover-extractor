...
try:
    from .config import (
        required_similarity_score, sleep_timer_bk, bookwalker_webhook_urls,
        bookwalker_logo_url, log_to_file, discord_embed_limit, grey_color,
        preorder_blue_color, paths, download_folders, paths_with_types,
        manga_extensions, novel_extensions
    )
except ImportError:
    print("WARN: Could not import from .config, using placeholder values in bookwalker_utils.")
...
try:
    ...
    def write_to_file(*args, **kwargs): pass # No-op
...
try:
    from .string_utils import (
        get_shortened_title, clean_str, similar, get_release_number_cache,
        get_file_part, is_one_shot, remove_brackets, contains_brackets,
        get_sort_key, normalize_str, unidecode # Added normalize_str, unidecode
    )
except ImportError:
     print("WARN: Could not import from .string_utils, defining placeholders.")
     def get_shortened_title(t): return t
     def clean_str(s, **kwargs): return s.lower().strip()
     def similar(a, b): return 0.0
     def get_release_number_cache(f, **kwargs): return ""
     def get_file_part(f, **kwargs): return ""
     def is_one_shot(f, **kwargs): return False
     def remove_brackets(s): return s
     def contains_brackets(s): return False
     def get_sort_key(n): return n
     def normalize_str(s, **kwargs): return s
     def unidecode(s): return s


try:
    from .file_utils import get_all_folders_recursively_in_dir, clean_and_sort, upgrade_to_file_class, get_folder_type # Added get_folder_type
except ImportError:
    print("WARN: Could not import from .file_utils, defining placeholders.")
    def get_all_folders_recursively_in_dir(p): return []
    def clean_and_sort(r, files=[], dirs=[], **kwargs): return files, dirs
    def upgrade_to_file_class(f, r, **kwargs): return [] # Needs File model
    def get_folder_type(files, **kwargs): return 0

try:
    from .models import BookwalkerBook, BookwalkerSeries, Volume, File, Publisher, Embed # Import needed models
except ImportError:
    print("WARN: Could not import from .models, defining placeholder classes.")
    class BookwalkerBook: pass
    class BookwalkerSeries: pass
    class Volume: pass
    class File: pass
...
def get_all_matching_books(books, book_type, title):
    ...
    for m in matching:
        if m in books: books.remove(m) # Ensure removal from original list
    return matching
...
def search_bookwalker(
    ...
    category_code = categories.get(book_type_code if not alternative_search else 'm_alt', categories['m']) # Default to manga
    ...
    if not alternative_search:
        try:
            series_page_soup = scrape_url(f"{series_check_url}{category_code}", cookies=default_cookies, headers=default_headers)
            if series_page_soup:
                series_list_ul = series_page_soup.find("ul", class_="o-tile-list")
                if series_list_ul:
                    series_list_count = len(series_list_ul.find_all("li", class_="o-tile"))
                    print(f"\t\tFound {series_list_count} series results for '{query}'.")
    ...
    if series_list_count == 1:
        print("\t\tOnly one series found, slightly lowering similarity requirement for volume matching.")
        current_required_similarity_score = required_similarity_score - 0.03
    elif shortened_search and series_list_count != 1:
         print("\t\t\tShortened search requires exactly one series result, skipping...\n")
    ...
    while current_page <= max_pages_to_scrape:
        ...
        print(f"\t\t\tScraping Page: {current_page} / {max_pages_to_scrape} ({search_url})")
        page_soup = scrape_url(search_url, cookies=default_cookies, headers=default_headers)

        if not page_soup:
            if book_type_code == 'm' and not alternative_search:
                 print("\t\t\tPrimary manga category failed, trying alternative...")
                 alt_results = search_bookwalker(query, book_type_code, print_info, alternative_search=True, shortened_search=shortened_search, total_pages_to_scrape=total_pages_to_scrape)
        ...
        if current_page == 1:
            ...
            if pager_area:
                page_numbers = [int(li.text) for li in pager_area.select("ul.clearfix > li") if li.text.isdigit()]
                if page_numbers:
        ...
        for item_index, item in enumerate(book_tiles):
            try:
                ...
                if not title_tag or not link_tag or not img_tag or not tag_box: continue
                ...
                is_chapter = any(t.find("div", class_=["a-tag-chapter", "a-tag-simulpub"]) for t in tags)
                book_type_tag = next((t.text.strip() for t in tags if t.find("div", class_=["a-tag-manga", "a-tag-light-novel", "a-tag-other"])), "Unknown")

                if is_chapter:
                    chapter_releases.append(original_title)
                    continue

                cleaned_title_for_match = clean_str(original_title, skip_bracket=True, skip_punctuation=True)
                cleaned_query_for_match = clean_str(query, skip_bracket=True, skip_punctuation=True)

                volume_number = get_release_number_cache(original_title)
                part = get_file_part(original_title)
                series_name_from_title = get_series_name_from_volume(original_title, root=None, test_mode=True)
                ...
                score_short = similar(clean_str(short_title), clean_str(short_query)) if short_title and short_query else 0.0

                if score < current_required_similarity_score and score_short < current_required_similarity_score:
                     if f'"{cleaned_title_for_match}": {score:.2f} [{book_type_tag}]' not in similarity_match_failures:
                          similarity_match_failures.append(f'"{cleaned_title_for_match}": {score:.2f} [{book_type_tag}]')
                     continue
                ...
                if volume_number == "":
                    if is_one_shot(original_title, skip_folder_check=True):
                        ...
                        if original_title not in no_volume_number: no_volume_number.append(original_title)
                        continue

                # --- Scrape individual book page ---
                book_page_soup = scrape_url(book_url, cookies=default_cookies, headers=default_headers)
                release_date_str = ""
                is_released = None
                description = ""
                ...
                if book_page_soup:
                    ...
                    elif not preview_image_url or not preview_image_url.startswith("http"):
                    ...
                    date_td = next((td for td in book_page_soup.select("table.product-detail td") if re.search(r"\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b", td.text)), None)
                    if date_td:
                        date_match = re.search(r"(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{1,2}),?\s+(\d{4})", date_td.text, re.IGNORECASE)
                        if date_match:
                            try:
                                date_obj = datetime.strptime(date_match.group(), "%b %d %Y")
                                release_date_str = date_obj.strftime("%Y-%m-%d")
                                is_released = date_obj < datetime.now()
                            except ValueError: pass

                book = BookwalkerBook(
                    series_name_from_title, original_title, volume_number, part,
                    release_date_str, is_released, 0.00, book_url, thumbnail,
                    book_type_tag, description, preview_image_url
                )
                books.append(book)
                time.sleep(sleep_timer_bk / 4) # Small delay

            except Exception as e:
                send_message(f"Error processing item {item_index} on page {current_page} ({book_url if 'book_url' in locals() else 'unknown URL'}): {e}", error=True)
                errors.append(f"Item {item_index}, Page {current_page}: {e}")
                continue

        if not book_tiles: break # Stop if no more books on page
        current_page += 1
    ...
    while temp_books:
        ...
        matching = get_all_matching_books(temp_books, book.book_type, book.title) # Use local helper
        current_series_books = [book] + matching
        series_list.append(BookwalkerSeries(book.title, current_series_books, len(current_series_books), book.book_type)) # Use imported model
    ...
    if len(series_list) == 1:
        series_list[0].books.sort(key=lambda x: (get_sort_key(x.volume_number), str(x.part)))
        return series_list[0].books
    elif len(series_list) > 1:
        print(f"\t\t\tWarning: Found multiple series matching '{query}'. Returning empty list.")
        return []
    else:
...
def check_for_new_volumes_on_bookwalker():
    ...
    def print_item_info(item):
        ...
        print(f"\t\tVolume {item.volume_number}" + (f" Part {item.part}" if item.part else ""))
        print(f"\t\tDate: {item.date}")
        print(f"\t\tURL: {item.url}\n")

    # Helper: Logs item info to a file
    def log_item_info(item, file_name):
        message = f"{item.date} | {item.original_title} | Volume {item.volume_number}" + (f" Part {item.part}" if item.part else "") + f" | {item.book_type} | {item.url}"
        write_to_file(f"{file_name.lower().replace(' ', '_')}.txt", message, without_timestamp=True, check_for_dup=True)

    # Helper: Creates a Discord embed for a BookwalkerBook
    def create_embed(item, color):
        embed = DiscordEmbed(title=f"{item.original_title} - Vol {item.volume_number}" + (f" Pt {item.part}" if item.part else ""), color=color, url=item.url)
        fields = [
            {"name": "Type", "value": item.book_type, "inline": False},
            {"name": "Release Date", "value": item.date if item.date else "Unknown", "inline": False},
        ]
        if item.description:
            desc = unidecode(item.description)
            max_len = 500
            fields.append({"name": "Description", "value": (desc[:max_len] + "...") if len(desc) > max_len else desc, "inline": False})

        embed = handle_fields(embed, fields)

        if item.preview_image_url: embed.set_image(url=item.preview_image_url)
        if bookwalker_logo_url: embed.set_author(name="Bookwalker", url="https://global.bookwalker.jp/", icon_url=bookwalker_logo_url)
    ...
    def process_items(items, list_name, color, webhook_index):
        ...
        webhook_url = bookwalker_webhook_urls[webhook_index] if bookwalker_webhook_urls and len(bookwalker_webhook_urls) > webhook_index else None
        for item in items:
            print_item_info(item)
            if log_to_file: log_item_info(item, list_name)
            if webhook_url:
                embed_wrapper = create_embed(item, color)
                current_notifications = group_notification(current_notifications, embed_wrapper, passed_webhook=webhook_url)

        if current_notifications and webhook_url:
            send_discord_message(None, embeds=current_notifications, passed_webhook=webhook_url)
    ...
    def determine_volume_type(volume_list):
        ...
        manga_percent = get_folder_type([vol.name for vol in volume_list], extensions=manga_extensions) # Use imported file_utils function
        novel_percent = get_folder_type([vol.name for vol in volume_list], extensions=novel_extensions) # Use imported file_utils function

        if manga_percent >= 70: return 'm' # Threshold from original logic
        if novel_percent >= 70: return 'l'
        ...
        if manga_count / total >= 0.7: return 'm'
        if novel_count / total >= 0.7: return 'l'
        return None # Mixed or unknown
    ...
    def filter_bookwalker_volumes(library_volumes, bookwalker_volumes):
        if not library_volumes or not bookwalker_volumes: return bookwalker_volumes

        # Create a set of existing (number, part) tuples for faster lookup
        existing_set = set()
        for vol in library_volumes:
            num = vol.index_number # Assumes index_number combines vol and part correctly
            if num != "":
                try:
                    existing_set.add(float(num))
                except (ValueError, TypeError): pass # Ignore if index_number isn't numeric

        # Filter bookwalker list
        filtered_bw_volumes = []
        for bw_vol in bookwalker_volumes:
            try:
                 bw_num = float(bw_vol.volume_number) + (float(bw_vol.part)/10 if bw_vol.part else 0.0)
    ...
    for path_index, path in enumerate(paths_to_scan, start=1):
        ...
        try:
            series_dirs = [entry.name for entry in scandir.scandir(path) if entry.is_dir() and not entry.name.startswith('.')]
        except Exception as e:
        ...
        for dir_index, series_dir in enumerate(series_dirs, start=1):
            ...
            try:
                ...
                series_files, _ = clean_and_sort(root, files=series_files, chapters=False, sort=True)
                ...
                volumes = upgrade_to_volume_class(
                    ...
                    skip_release_year=True, skip_file_part=False, skip_release_group=True,
                    skip_extras=True, skip_publisher=True, skip_premium_content=True,
                    skip_subtitle=True, skip_multi_volume=False
                )
                ...
                series_name_query = normalize_str(series_dir, skip_type_keywords=True, skip_editions=True)
                series_name_query = unidecode(series_name_query)

                bookwalker_volumes = search_bookwalker(series_name_query, volume_type)

                if not bookwalker_volumes: continue

                missing_volumes = filter_bookwalker_volumes(volumes, bookwalker_volumes)

                if missing_volumes:
                    print(f"\t\tFound {len(missing_volumes)} potential new/missing volumes on Bookwalker.")
                    print_releases(missing_volumes, all_released, all_pre_orders) # Use local helper
