...
try:
    ...
    print(
        "WARN: Could not import from .config, using placeholder values in core_logic."
    )
...
try:
    ...
    from .file_utils import (
        get_lines_from_file,
        cache_path,
    )  # Import only what main needs directly
    ...
    from .archive_utils import (
        convert_to_cbz,
    )  # Moved convert_to_cbz here from file_operations import
...
def is_root_present(root_path, target_path):
    ...
    return (
        target_path_norm.startswith(root_path_norm + "/")
        or target_path_norm == root_path_norm
    )
...
def main(folders_to_process=None):
    ...
    if (
        ...
        and os.path.isfile(cached_paths_path)
    ):
        cached_paths = get_lines_from_file(
            cached_paths_path, ignore=paths + download_folders, check_paths=True
        )
    ...
    if (
        (cache_each_root_for_each_path_in_paths_at_beginning_toggle or not cached_paths)
        and paths
    ...
    process_target_folders = (
        folders_to_process if folders_to_process is not None else download_folders
    )

    if correct_file_extensions_toggle and process_target_folders:
        correct_file_extensions(
            folders=process_target_folders
        )  # Call placeholder/imported function
    ...
    if rename_files_in_download_folders_toggle and process_target_folders:
        rename_files(
            download_folders=process_target_folders
        )  # Call placeholder/imported function
    ...
    if check_for_duplicate_volumes_toggle and process_target_folders:
        check_for_duplicate_volumes(
            paths_to_search=process_target_folders
        )  # Call placeholder/imported function
    ...
    if (
        extract_covers_toggle
        and paths
        and download_folder_in_paths
        and process_target_folders
    ):
        extract_covers(
            paths_to_process=process_target_folders
        )  # Call placeholder/imported function
    ...
    if check_for_existing_series_toggle and process_target_folders and paths:
        ...
        check_for_existing_series(
            test_download_folders=process_target_folders
        )  # Call placeholder/imported function
    ...
    if library_paths_to_process:  # Only proceed if there are library paths to process
        if move_series_to_correct_library_toggle and paths_with_types:
            move_series_to_correct_library(
                paths_to_search=library_paths_to_process
            )  # Call placeholder/imported function

        if extract_covers_toggle:
            extract_covers(
                paths_to_process=library_paths_to_process
            )  # Call placeholder/imported function
        ...
        if komga_libraries:  # Ensure libraries were fetched successfully
            for path in moved_files:
                if not os.path.exists(path):
                    continue  # Skip if file was deleted/moved again
                for library in komga_libraries:
                    if library["id"] in scanned_libs:
                        continue
