...
from .config import (
    image_extensions, novel_extensions, output_covers_as_webp,
    compress_image_option, image_quality,
    blank_white_image_path, blank_black_image_path,
...
try:
    from .models import File # Expected type for 'file' parameter in find_and_extract_cover
except ImportError:
    print("WARN: Could not import File model in image_utils.py")
    class File: pass # Placeholder

# Import needed function from metadata_utils
try:
    from .metadata_utils import get_novel_cover_path # Needed for find_and_extract_cover
except ImportError:
...
def compress_image(image_path, quality=image_quality, to_jpg=False, raw_data=None): # Use imported config value
    ...
    try:
        ...
        if output_covers_as_webp: # Use imported config value
            ...
            save_format = "JPEG"
            ext = ".jpg"
            if not to_jpg: to_jpg = True # Flag for potential original PNG removal
        ...
        if not raw_data:
            ...
            buffer = io.BytesIO()
            image.save(buffer, format=save_format, quality=quality, optimize=True) # Added optimize=True
            return buffer.getvalue()

    except Exception as e:
        send_message(f"Failed to compress image {image_path}: {e}", error=True) # Use imported log_utils function
        return None if raw_data else image_path # Return original path or None on failure

    # Remove the original file if it was a PNG converted to JPG
    if to_jpg and ext.lower() == ".jpg" and image_path.lower().endswith(".png") and os.path.isfile(image_path):
...
def is_image_black_and_white(image, tolerance=15):
    ...
    try:
        ...
        send_message(f"Error checking if image is black and white: {e}", error=True) # Use imported log_utils function
        return False
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zip_file:
            ...
            image_files = sorted([
                ...
                if not f.endswith('/') and get_file_extension(f) in image_extensions # Use imported config/file_utils
            ])
            if not image_files: return False # No images in archive
        ...
        send_message(f"Bad zip file, cannot check first image: {zip_path}", error=True) # Use imported log_utils function
        return False # Treat as not black and white if unreadable
    except Exception as e:
        send_message(f"Error processing zip file {zip_path}: {e}", error=True) # Use imported log_utils function
        return False
    ...
    try:
        with zipfile.ZipFile(file_path, "r") as archive:
            images = [
                ...
                if get_file_extension(f).lower() in image_extensions # Use imported config/file_utils
            ]
            return len(images)
    except zipfile.BadZipFile:
        send_message(f"Skipping corrupted file for image count: {file_path}", error=True) # Use imported log_utils function
        return 0
    except Exception as e:
        send_message(f"Error counting images in {file_path}: {e}", error=True) # Use imported log_utils function
        return 0
...
compiled_cover_patterns = [re.compile(pattern, flags=re.IGNORECASE) for pattern in cover_patterns]
...
def find_and_extract_cover(
    ...
    def filter_and_sort_files(zip_list):
        return sorted([
            x for x in zip_list
            if not x.endswith('/') and "." in x and
               get_file_extension(x).lower() in image_extensions and # Use imported config/file_utils
               not os.path.basename(x).startswith((".", "__"))
        ])
    ...
    def process_cover_image(cover_path, image_data=None):
        if not image_data: return None # Cannot process without data

        image_extension = get_file_extension(os.path.basename(cover_path)).lower() # Use imported file_utils function
        if image_extension == ".jpeg": image_extension = ".jpg"

        output_ext = ".webp" if output_covers_as_webp else ".jpg" # Use imported config value
        ...
        if compress_image_option or output_ext != image_extension: # Use imported config value
            # Pass output_path to compress_image for correct extension handling if saving
            compressed_data = compress_image(output_path, raw_data=image_data) # Use local function
            if return_data_only:
                return compressed_data if compressed_data else image_data # Return compressed or original data
            else:
    ...
    def is_blank_image(img_data):
        if not blank_image_check: return False # Skip check if disabled
        if not blank_white_image_path and not blank_black_image_path: return False # Skip if no blank refs

        is_blank = False
        if blank_white_image_path:
            score_white = prep_images_for_similarity(blank_white_image_path, img_data, silent=True) # Use local function
            if score_white is not None and score_white >= blank_cover_required_similarity_score: # Use imported config value
                is_blank = True
        if not is_blank and blank_black_image_path: # Only check black if white didn't match
            score_black = prep_images_for_similarity(blank_black_image_path, img_data, silent=True) # Use local function
            if score_black is not None and score_black >= blank_cover_required_similarity_score: # Use imported config value
                is_blank = True

        if is_blank and not silent: print("\t\t\tDetected blank image, skipping.")
        return is_blank

    # --- Main function logic ---
    if not os.path.isfile(file.path):
        if not silent: send_message(f"File not found: {file.path}", error=True) # Use imported log_utils function
        return None

    if not zipfile.is_zipfile(file.path):
        if not silent: send_message(f"Not a valid zip file: {file.path}", error=True) # Use imported log_utils function
        return None
    ...
    try:
        with zipfile.ZipFile(file.path, "r") as zip_ref:
            ...
            if novel_cover_rel_path:
                novel_cover_full_path = next((item for item in zip_list if os.path.basename(item) == os.path.basename(novel_cover_rel_path)), None)
                if novel_cover_full_path:
                    prioritized_list = [novel_cover_full_path] + [item for item in zip_list if item != novel_cover_full_path]

            # Try pattern matching first
            for image_file in prioritized_list:
                image_basename = os.path.basename(image_file)
                is_novel_cover = novel_cover_rel_path and image_basename == os.path.basename(novel_cover_rel_path)

                match_found = is_novel_cover or any(pattern.search(image_basename) for pattern in compiled_cover_patterns)
                ...
                if match_found:
                    ...
                    if not is_blank_image(image_data):
                        ...
                        if result:
                            if not return_data_only and not silent: print(f"\t\tCover extracted: {os.path.basename(result)}")
            ...
            if not silent: print("\t\tNo pattern matched, using first valid image as cover.")
            for image_file in prioritized_list: # Use prioritized list for fallback too
                 image_data = get_image_data(zip_ref, image_file)
                 if not is_blank_image(image_data):
                     result = process_cover_image(image_file, image_data)
                     if result:
                         if not return_data_only and not silent: print(f"\t\tCover extracted (fallback): {os.path.basename(result)}")
                         # image_count += 1 # TODO: Handle stats counting elsewhere
                         return result
                     else: # If processing the first valid image fails, report error
                          if not silent: send_message(f"Failed to process fallback cover: {image_file}", error=True) # Use imported log_utils function
        ...
        if not silent: send_message(f"Bad zip file: {file.path}", error=True) # Use imported log_utils function
        return None
    except Exception as e:
        if not silent: send_message(f"Error extracting cover from {file.path}: {e}", error=True) # Use imported log_utils function
        return None
...
def convert_webp_to_jpg(webp_file_path):
    ...
    if not webp_file_path or not webp_file_path.lower().endswith(".webp"): return None
    if not os.path.isfile(webp_file_path): return None

    extensionless_webp_file = get_extensionless_name(webp_file_path) # Use imported file_utils function
    ...
    try:
        with Image.open(webp_file_path) as im:
            ...
            im.convert("RGB").save(jpg_file_path, "JPEG", quality=95) # Save with high quality

        if os.path.isfile(jpg_file_path):
            remove_file(webp_file_path, silent=True) # Use imported file_utils function
            return jpg_file_path
        else:
            send_message(f"ERROR: Failed to verify JPG creation for {webp_file_path}", error=True) # Use imported log_utils function
            return None
    except Exception as e:
        send_message(f"Could not convert {webp_file_path} to jpg: {e}", error=True) # Use imported log_utils function
        # Clean up partially created JPG if conversion failed
        if os.path.isfile(jpg_file_path): remove_file(jpg_file_path, silent=True) # Use imported file_utils function
        return None
...
def preprocess_image(image):
    ...
    try:
        ...
        if isinstance(image, bytes):
             ...
             if img_np is None: raise ValueError("cv2.imdecode returned None")
             image = img_np
        ...
        send_message(f"Error preprocessing image: {e}", error=True) # Use imported log_utils function
        return None
...
def compare_images(imageA, imageB, silent=False):
    ...
    try:
        ...
        if not silent: print(f"\t\t\t\tSSIM: {ssim_score:.4f}")
        return ssim_score
         ...
         if "input images must have the same dimensions" in str(ve):
             if not silent: print("\t\t\tImage dimension mismatch for SSIM.")
         else:
             send_message(f"SSIM calculation error: {ve}", error=True) # Use imported log_utils function
         return 0.0 # Return 0 similarity on error
    except Exception as e:
        send_message(f"Error comparing images: {e}", error=True) # Use imported log_utils function
        return 0.0 # Return 0 similarity on error
...
def prep_images_for_similarity(
    ...
    def resize_image(img, desired_width=400, desired_height=600):
        ...
        return cv2.resize(img, (desired_width, desired_height), interpolation=cv2.INTER_AREA)
    ...
    try:
        ...
        if both_cover_data: # image1 is data
            ...
            if not os.path.isfile(image1_path_or_data):
                 if not silent: print(f"\t\t\tReference image not found: {image1_path_or_data}")
                 return None
        ...
        if img1 is None or img2 is None:
            if not silent: print("\t\t\tFailed to load one or both images for comparison.")
            return None
        ...
        send_message(f"Error preparing images for similarity: {e}", error=True) # Use imported log_utils function
        return None