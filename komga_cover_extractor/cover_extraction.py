...
import os
import shutil
import time  # Added for watchdog sleep
import zipfile
import re
...
from skimage.metrics import structural_similarity as ssim  # Added for image comparison
import scandir  # Added for directory walking
...
try:
    ...
    print(
        "WARN: Could not import from .config, using placeholder values in cover_extraction."
    )
    paths = []
    download_folders = []
    paths_with_types = []
...
try:
    ...
    def get_modification_date(p):
        return 0

    def set_modification_date(*args, **kwargs):
        pass

    def get_file_hash(p, *args):
        return None
    ...
    def get_folder_type(*args, **kwargs):
        return 0
    ...
    def convert_webp_to_jpg(*args, **kwargs):
        return None
...
file_counters = {
    ext: 0 for ext in image_extensions | set(file_extensions)
}  # Stats counter
    ...
    if use_latest_volume_cover_as_series_cover and not is_chapter_directory:
        ...
        for item in releases:
            ...
            if isinstance(current_item, list):
                current_item = tuple(current_item)
...
def process_cover_extraction(
    ...
    def filter_series_by_first_word(filtered_series, first_word):
        return [
            ...
            if clean_str(folder)
            .lower()
            .startswith(first_word)  # Use imported string_utils function
        ]
    ...
    try:
        ...
        cover_extension = (
            get_file_extension(cover) if cover else ""
        ...
        if cover_extension:
            ...
            if needs_conversion:
                ...
                if remove_file(cover, silent=True):  # Use imported file_utils function
                    ...
                    send_message(
                        f"Failed to remove existing cover '{os.path.basename(cover)}' for conversion.",
                        error=True,
                    )
        ...
        if not cover:
            ...
            if result:
                ...
                if result.lower().endswith(".webp") and not output_covers_as_webp:
                    ...
                    conversion_result = convert_webp_to_jpg(
                        result
                    )  # Use imported image_utils function
                    if conversion_result:
                        print("\t\tCover successfully converted to JPG.")
                        cover = conversion_result
                    else:
                        print(
                            "\t\tCover conversion failed. Cleaning up extracted WebP."
                        )
                    ...
                    result.lower().endswith((".jpg", ".jpeg", ".png"))
                    and output_covers_as_webp
                ):
                    # TODO: Implement JPG/PNG to WebP conversion if needed after extraction
                    print(
                        f"\t\tExtracted {get_file_extension(result)}, desired WebP (conversion not implemented yet). Using original."
                    )
                ...
                if cover:
                    print(
                        f"\t\tCover successfully extracted/processed: {os.path.basename(cover)}\n"
                    )
                    has_cover = True
        ...
        should_update_series_cover = (
            ...
            and cover  # A valid cover must exist for this volume
            and series_cover_path  # A series cover must already exist
        ...
        if should_update_series_cover:
            try:
                current_series_cover_mod_date = get_modification_date(
                    series_cover_path
                )  # Use imported file_utils function
                volume_cover_mod_date = get_modification_date(
                    cover
                )  # Use imported file_utils function

                if current_series_cover_mod_date != volume_cover_mod_date:
                    current_series_hash = get_file_hash(
                        series_cover_path
                    )  # Use imported file_utils function
                    volume_cover_hash = get_file_hash(
                        cover
                    )  # Use imported file_utils function

                    if current_series_hash != volume_cover_hash:
                        print(
                            "\t\tSeries cover differs from designated volume cover. Replacing..."
                        )
                        if remove_file(
                            ...
                            try:
                                shutil.copy2(
                                    cover, series_cover_path
                                )  # copy2 preserves metadata like mod time
                                print(
                                    f"\t\tReplaced series cover with: {os.path.basename(cover)}"
                                )
                            except Exception as copy_err:
                                send_message(
                                    f"Failed to copy new series cover: {copy_err}",
                                    error=True,
                                )
                                series_cover_path = ""  # Invalidate path if copy failed
                        else:
                            send_message(
                                "Failed to remove old series cover for replacement.",
                                error=True,
                            )
                        ...
                        print(
                            "\t\tSeries cover hash matches volume cover. Updating modification time."
                        )
                        set_modification_date(
                            series_cover_path, volume_cover_mod_date
                        )  # Use imported file_utils function
            except Exception as series_cover_err:
                send_message(
                    f"Error comparing/updating series cover: {series_cover_err}",
                    error=True,
                )
        ...
        if (
            ...
            print(
                f"\tAttempting to copy volume cover for chapter series: {clean_basename}"
            )
            found_volume_cover_source = False
            for v_path_obj in volume_paths:
                if found_volume_cover_source:
                    break  # Stop if cover found

                # Filter potential source folders
                source_series_folders = getattr(
                    v_path_obj, "series_folders", []
                )  # Get pre-filtered list if available
                if not source_series_folders:
                    continue  # Skip if no folders for this path

                first_word = get_first_word(clean_basename)  # Use local helper
                if first_word:
                    source_series_folders = filter_series_by_first_word(
                        source_series_folders, first_word
                    )  # Use local helper

                for folder_name in source_series_folders:
                    if (
                        similar(clean_str(folder_name), clean_basename)
                        ...
                        potential_cover = next(
                            (
                                ...
                                if os.path.isfile(
                                    os.path.join(
                                        volume_series_path, f"{cover_name}{ext}"
                                    )
                                )
                            ),
                            None,
                        ...
                        if potential_cover:
                            target_series_cover_path = os.path.join(
                                file.root, f"cover{get_file_extension(potential_cover)}"
                            )  # Use imported file_utils function
                            copy_needed = True
                            if os.path.isfile(target_series_cover_path):
                                # Compare hashes if target exists
                                if get_file_hash(
                                    target_series_cover_path
                                ) == get_file_hash(
                                    potential_cover
                                ):  # Use imported file_utils function
                                    copy_needed = False
                                    print(
                                        "\t\tChapter series already has matching cover."
                                    )
                                else:
                                    print(
                                        "\t\tChapter series cover differs. Replacing."
                                    )
                                    remove_file(
                                        target_series_cover_path, silent=True
                                    )  # Use imported file_utils function

                            if copy_needed:
                                try:
                                    shutil.copy2(
                                        potential_cover, target_series_cover_path
                                    )  # copy2 preserves metadata
                                    print(
                                        f"\t\tCopied cover from '{folder_name}' to chapter series."
                                    )
                                    series_cover_path = (
                                        target_series_cover_path  # Update global path
                                    )
                                    found_volume_cover_source = True
                                    break  # Found cover, stop searching this path obj
                                except Exception as copy_err:
                                    send_message(
                                        f"Failed to copy cover from volume library: {copy_err}",
                                        error=True,
                                    )
        ...
        should_create_series_cover = (
            not is_chapter_directory
            and has_cover
            and cover  # Ensure cover extraction was successful
            and not series_cover_path  # Only if series cover doesn't exist
            and not contains_subfolders  # Don't create in root if subfolders exist
            and file.root not in download_folders  # Don't create in download folders
            and not has_multiple_volume_ones
            and same_series_name  # Ensure consistency within the folder
            and (
                (
                    use_latest_volume_cover_as_series_cover
                    and is_same_index_number(
        ...
        if should_create_series_cover:
            ...
            target_series_cover_path = os.path.join(
                file.root, f"cover{get_file_extension(cover)}"
            )  # Use imported file_utils function
            try:
                shutil.copy2(
                    cover, target_series_cover_path
                )  # copy2 preserves metadata
                print(
                    f"\t\tCreated series cover: {os.path.basename(target_series_cover_path)}"
                )
        ...
        try:
            for root, dirs, files in scandir.walk(path):
                ...
                if watchdog_toggle:
                    ...
                    root_mod_time = get_modification_date(
                        root
                    )  # Use imported file_utils function
                    if (
                        root in root_modification_times
                        and root_modification_times[root] == root_mod_time
                    ):
                ...
                if series_cover_path:
                    ...
                    if needs_conversion:
                        print(
                            f"\tSeries cover '{os.path.basename(series_cover_path)}' needs format conversion."
                        )
                        if remove_file(
                            series_cover_path, silent=True
                        ):  # Use imported file_utils function
                            ...
                            send_message(
                                "Failed to remove series cover for conversion.",
                                error=True,
                            )

                is_chapter_directory = (
                    get_folder_type(volume_objects, file_type="chapter") >= 90
                )  # Use imported file_utils function
                same_series_name = (
                    len(
                        set(
                    ...
                    <= 1
                )  # Check if all series names are the same (or only one unique name exists)
                    ...
                    if not volume_paths:  # Initialize only once
                        ...
                        for vp in volume_paths:
                            try:
                                vp.series_folders = [
                                    entry.name
                                    for entry in scandir.scandir(vp.path)
                                    if entry.is_dir() and not entry.name.startswith(".")
                                ]
                            except Exception as scan_err:
                                send_message(
                                    f"Error scanning volume path {vp.path}: {scan_err}",
                                    error=True,
                                )
                                vp.series_folders = []
                    base_name = clean_str(
                ...
                try:
                    ...
                    send_message(
                        f"Error determining highest release in {root}: {high_err}",
                        error=True,
                    )
                    highest_index_number = ""

                has_multiple_volume_ones = (
                    not use_latest_volume_cover_as_series_cover or is_chapter_directory
                ) and sum(
                    1
                    for vol in volume_objects
                    if not vol.is_one_shot
                    and not vol.volume_part
                    and (
                        vol.index_number == 1
                        or (
                            isinstance(vol.index_number, list) and 1 in vol.index_number
                        )
                    )
                ) > 1
                ...
                for file_obj in volume_objects:
                    ...
                    if file_obj.file_type == "volume" or (
                        file_obj.file_type == "chapter" and extract_chapter_covers
                    ):
            ...
            send_message(
                f"Error walking directory {path}: {e}\n{traceback.format_exc()}",
                error=True,
            )
