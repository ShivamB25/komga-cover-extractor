...
try:
    from .config import (
        download_folders, paths, paths_with_types, file_extensions,
        unacceptable_keywords, manga_extensions, exception_keywords,
        manual_rename, rename_zip_to_cbz, LOGS_DIR,
        ...
        # Colors
        yellow_color, grey_color,
        # Toggles
        watchdog_toggle, send_scan_request_to_komga_libraries_toggle
    )
except ImportError:
    print("WARN: Could not import from .config, using placeholder values in file_operations.")
    download_folders = []
    paths = []
    paths_with_types = []
...
try:
    ...
    from .file_utils import (
        process_files_and_folders, get_file_extension, get_header_extension,
        get_extensionless_name, rename_file, remove_file, move_file,
        create_folder_obj, upgrade_to_file_class, check_and_delete_empty_folder,
        get_library_type, move_folder # Added move_folder
    )
    from .string_utils import (
        contains_chapter_keywords, contains_volume_keywords,
        check_for_exception_keywords, similar, clean_str, get_series_name,
        get_series_name_from_volume, remove_brackets # Added remove_brackets
    )
    ...
    def create_folder_obj(*args, **kwargs): return None # Needs Folder model
    def upgrade_to_file_class(*args, **kwargs): return [] # Needs File model
    def check_and_delete_empty_folder(*args, **kwargs): pass
    def get_library_type(*args, **kwargs): return None
    def move_folder(*args, **kwargs): return False
    def contains_chapter_keywords(*args, **kwargs): return False
    def contains_volume_keywords(*args, **kwargs): return False
    def check_for_exception_keywords(*args, **kwargs): return False
    def similar(a, b): return 0.0
    def clean_str(s, **kwargs): return s
    def get_series_name(d): return d
    def get_series_name_from_volume(*args, **kwargs): return ""
    def remove_brackets(s): return s
    def get_input_from_user(*args, **kwargs): return "y" # Default to yes for placeholders
        ...
        try:
            for root, dirs, files in scandir.walk(folder):
                ...
                files, dirs = process_files_and_folders(
                    ...
                    is_correct_extensions_feature=file_extensions + [".rar", ".cbr", ".7z"] # Include convertable
                )
                ...
                volumes = upgrade_to_file_class(
                    ...
                    is_correct_extensions_feature=file_extensions + [".rar", ".cbr", ".7z"]
                )

                if not volumes: continue

                for volume in volumes:
                    if not volume.header_extension: continue # Skip if header couldn't be determined
                    ...
                    if volume.extension.lower() != volume.header_extension.lower():
                        new_name = f"{volume.extensionless_name}{volume.header_extension}"
                        new_path = os.path.join(volume.root, new_name)

                        print(f"\n\t\t\tRenaming File:\n\t\t\t\t{volume.name}\n\t\t\t\t\tto\n\t\t\t\t{new_name}")

                        user_input = get_input_from_user("\t\t\tRename", ["y", "n"], ["y", "n"]) if manual_rename else "y"

                        if user_input == "y":
                            rename_status = rename_file(volume.path, new_path, silent=True) # Use imported file_utils function
                            if rename_status:
                                ...
                                if watchdog_toggle:
                                    if volume.path in transferred_files: transferred_files.remove(volume.path)
                                    if new_path not in transferred_files: transferred_files.append(new_path)
                            else:
                                 send_message(f"Failed to rename {volume.name} to {new_name}", error=True)
                        else:
                            print("\t\t\tSkipped")
        except Exception as e:
             send_message(f"Error scanning {folder} for extension correction: {e}", error=True)
...
def delete_unacceptable_files(folders=download_folders):
    ...
    try:
        for path in folders:
            ...
            for root, dirs, files in scandir.walk(path):
                ...
                for file in files:
                    ...
                    for keyword_pattern in unacceptable_keywords:
                        try:
                            unacceptable_keyword_search = re.search(keyword_pattern, file, re.IGNORECASE)
                            if unacceptable_keyword_search:
                                send_message(f"\tUnacceptable: '{unacceptable_keyword_search.group()}' match found in {file}\n\t\tDeleting file from: {root}", discord=False)
                                # TODO: Add Discord notification via discord_utils
                                # embed = handle_fields(...)
                                # grouped_notifications = group_notification(...)
                             ...
                             send_message(f"Invalid regex pattern in unacceptable_keywords: '{keyword_pattern}'. Error: {re_err}", error=True)
            ...
            for root, dirs, files in scandir.walk(path, topdown=False): # Walk bottom-up for cleanup
                 for folder_name in dirs:
                     check_and_delete_empty_folder(os.path.join(root, folder_name)) # Use imported file_utils function
        ...
        for path in folders:
            ...
            for root, dirs, files in scandir.walk(path):
                ...
                for file in files:
                    ...
                    is_chapter = contains_chapter_keywords(file) # Use imported string_utils function
                    is_volume = contains_volume_keywords(file) # Use imported string_utils function
                    is_exception = check_for_exception_keywords(file, exception_keywords) # Use imported string_utils function
                    is_manga = get_file_extension(file) in manga_extensions # Use imported file_utils function

                    if is_chapter and not is_volume and not is_exception and is_manga:
                        send_message(f"\n\t\tFile: {file}\n\t\tLocation: {root}\n\t\tContains chapter keywords/lone numbers and does not contain any volume/exclusion keywords\n\t\tDeleting chapter release.", discord=False)
                        # TODO: Add Discord notification via discord_utils
                        # embed = handle_fields(...)
                        # grouped_notifications = group_notification(...)
                        remove_file(file_path) # Use imported file_utils function
            ...
            # Clean up empty folders after deletion pass
            for root, dirs, files in scandir.walk(path, topdown=False): # Walk bottom-up
                 for folder_name in dirs:
                     check_and_delete_empty_folder(os.path.join(root, folder_name)) # Use imported file_utils function
                 if root == path:
                      check_and_delete_empty_folder(root)
...
def rename_files(download_folders=download_folders, test_mode=False):
    ...
    for path in download_folders:
        ...
        # Use scandir for potentially better performance
        for root, dirs, files in scandir.walk(path):
            if test_mode and root not in download_folders: continue # Limit test mode scope
            ...
            updated_volumes = reorganize_and_rename(volumes, root) # Use imported string_utils function
    ...
    for download_folder in folders:
        ...
        try:
            ...
            files, _ = process_files_and_folders(
                download_folder, top_level_files, [], # Pass empty dirs list
                just_these_files=transferred_files,
            ...
            for file_obj in file_objects:
                if not file_obj.basename: continue # Skip if series name couldn't be determined
                ...
                if move_lone_files_to_similar_folder and top_level_dirs:
                    similar_dirs = [
                        ...
                        if similar(clean_str(d), clean_str(file_obj.basename)) >= required_similarity_score
                    ]
                    if similar_dirs:
                        chosen_dir = similar_dirs[0] # Move to the first similar directory found
                        target_folder_path = os.path.join(download_folder, chosen_dir)
                        target_file_path = os.path.join(target_folder_path, file_obj.name)

                        # Optional: Rename file to match folder name if enabled
                        if replace_series_name_in_file_name_with_similar_folder_name and file_obj.basename != chosen_dir:
                             # TODO: Implement file renaming logic here if needed, similar to original script
                             pass # Placeholder for renaming logic

                        if not os.path.isfile(target_file_path):
                            if move_file(file_obj, target_folder_path): # Use imported file_utils function
                                moved = True
                                # Update watchdog state
                                if watchdog_toggle:
                                    if target_file_path not in transferred_files: transferred_files.append(target_file_path)
                                    if file_obj.path in transferred_files: transferred_files.remove(file_obj.path)
                                    # Add target folder to transferred_dirs if not already present
                                    if not any(td.root == target_folder_path for td in transferred_dirs):
                                         transferred_dirs.append(create_folder_obj(target_folder_path)) # Use imported file_utils function
                        else:
                             # File already exists in target, remove the source lone file
                             send_message(f"Duplicate found in {chosen_dir}, removing lone file: {file_obj.name}", discord=False)
                             remove_file(file_obj.path, silent=True) # Use imported file_utils function
                ...
                if not moved:
                    if not os.path.isdir(target_folder_path):
                        try:
                            ...
                            send_message(f"Failed to create folder {target_folder_path}: {e}", error=True)
                            continue
                    ...
                    if not os.path.isfile(target_file_path):
                        if move_file(file_obj, target_folder_path): # Use imported file_utils function
                         ...
                         send_message(f"Duplicate found in new folder {file_obj.basename}, removing lone file: {file_obj.name}", discord=False)
                         remove_file(file_obj.path, silent=True) # Use imported file_utils function
                         moved = True # Handled

        except Exception as e:
            send_message(f"Error processing download folder {download_folder}: {e}", error=True)
...
def move_series_to_correct_library(paths_to_search=paths_with_types):
    ...
    try:
        ...
        for pwt in paths_with_types:
            for lib_type_name in pwt.library_types: # Assuming library_types is list of names
                 # Store path based on format (volume/chapter) and type (manga/novel)
                 key = (tuple(sorted(pwt.path_formats)), lib_type_name) # Use formats and type as key
                 if key not in target_paths_map: target_paths_map[key] = []
        ...
        for current_path_obj in paths_to_search:
            ...
            try:
                ...
                series_dirs = [entry.path for entry in scandir.scandir(current_path) if entry.is_dir() and not entry.name.startswith('.')]
            except Exception as e:
            ...
            for series_dir_path in series_dirs:
                ...
                try:
                    ...
                    series_files = [entry.name for entry in scandir.scandir(series_dir_path) if entry.is_file()]
                    # Use process_files_and_folders for consistent filtering
                    files, _ = process_files_and_folders(series_dir_path, series_files, [])
                    ...
                    if not file_objects: continue

                    # Determine dominant library type and format (volume/chapter)
                    actual_library_type = get_library_type([fo.name for fo in file_objects]) # Use imported file_utils function
                    actual_format = "chapter" if get_folder_type(file_objects, file_type="chapter") >= 90 else "volume" # Use imported file_utils function

                    if not actual_library_type:
                        print("\t\tCould not determine dominant library type for series.")
                        continue
                    ...
                    current_path_lib_types = current_path_obj.library_types # Assuming list of names

                    is_correct_location = (actual_format in current_path_formats and
                                           actual_library_type.name in current_path_lib_types)
                    ...
                    target_key = (tuple(sorted([actual_format])), actual_library_type.name)
                    possible_target_paths = target_paths_map.get(target_key, [])

                    if not possible_target_paths:
                        print(f"\t\tNo suitable target library path found for type '{actual_library_type.name}' and format '{actual_format}'. Skipping move.")
                        continue
                    ...
                    print(f"\t\tMismatch detected: Series type '{actual_library_type.name}/{actual_format}' in path for '{'/'.join(current_path_lib_types)}/{'/'.join(current_path_formats)}'.")
                    ...
                    if os.path.isdir(new_location_dir):
                        ...
                        print(f"\t\tTarget directory '{new_location_dir}' already exists. Skipping move to prevent conflicts.")
                        # TODO: Implement merge logic or user prompt if needed
                        continue

                    # Move the folder
                    if move_folder(series_dir_path, target_path, silent=True): # Use imported file_utils function
                        send_message(f"\t\tMoved '{series_dir_name}' from '{current_path}' to '{target_path}'", discord=False)
                        # Update state for Komga scan
                        if new_location_dir not in moved_folders: moved_folders.append(new_location_dir)
                        # Add all files within the moved folder to moved_files
                        moved_files.extend([os.path.join(new_location_dir, f) for f in files])
                        ...
                        if send_scan_request_to_komga_libraries_toggle:
                            if not komga_libraries: komga_libraries = get_komga_libraries() # Use imported komga_utils function
                            if komga_libraries:
                                 libs_to_scan_now = set()
                                 for lib in komga_libraries:
                                     if is_root_present(lib["root"], current_path) or is_root_present(lib["root"], target_path): # Use helper
                                         libs_to_scan_now.add(lib["id"])
                                 for lib_id in libs_to_scan_now:
                                     if lib_id not in libraries_to_scan: libraries_to_scan.append(lib_id) # Add to global list for final scan
                    else:
                        send_message(f"\t\tFailed to move '{series_dir_name}' to '{target_path}'.", error=True)

                except Exception as series_err:
                    send_message(f"Error processing series directory {series_dir_path}: {series_err}", error=True)
...
def reorganize_and_rename(files, dir):
    ...
    modifiers = {
        ext: (
            "[%s]" if ext in novel_extensions else
            "(%s)" if ext in manga_extensions else
            " %s " # Default space padding if unknown
        )
    ...
    for file in files[:]:
        try:
            ...
            keywords, preferred_naming_format, zfill_int, zfill_float = (
                (chapter_regex_keywords, preferred_chapter_renaming_format, zfill_chapter_int_value, zfill_chapter_float_value) if file.file_type == "chapter" else
                (volume_regex_keywords, preferred_volume_renaming_format, zfill_volume_int_value, zfill_volume_float_value)
            )
            ...
            keyword_num_pattern = rf"\b({keywords})\s*(\d+([._-]\d+)?)\b" # Basic pattern
            if re.search(keyword_num_pattern, file.name, re.IGNORECASE) or file.volume_number != "":
                ...
                if file.multi_volume and isinstance(file.volume_number, list):
                    ...
                    for i, n_str in enumerate(numbers):
                         ...
                         if i < len(numbers) - 1: number_string += "-"
                ...
                if add_issue_number_to_manga_file_name and file.file_type == "volume" and file.extension in manga_extensions and number_string:
                ...
                # Add year
                if file.volume_year:
                    rename += f" {modifiers.get(file.extension, '(%s)') % file.volume_year}"
                    # Remove year from extras if it was added
                    file.extras = [
                        item for item in file.extras
                        if not (str(file.volume_year) in item or similar(item.strip("()[]{} "), str(file.volume_year)) >= 0.95 or re.search(r"([\[\(\{]\d{4}[\]\)\}])", item))
                    ]
                ...
                if publisher_name and add_publisher_name_to_file_name_when_renaming:
                    ...
                    for item in file.extras[:]:
                        ...
                        if similar(item_clean, publisher_name) >= publisher_similarity_score:
                            try:
                                file.extras.remove(item)
                            except ValueError: pass # Item might have already been removed
                    rename += f" {modifiers.get(file.extension, '[%s]') % publisher_name}"


                # Add premium tag
                if file.is_premium and search_and_add_premium_to_file_name:
                    rename += f" {modifiers.get(file.extension, '[%s]') % 'Premium'}"
                    file.extras = [item for item in file.extras if "premium" not in item.lower()]
                ...
                if file.release_group:
                    ...
                    if move_release_group_to_end_of_file_name:
                        ...
                        for item in file.extras[:]:
                             item_clean = re.sub(r"[\(\)\[\]{}]", "", item).strip()
                             if similar(item_clean, file.release_group) >= release_group_similarity_score:
                                 try:
                                     file.extras.remove(item)
                                 except ValueError: pass
                    # If not moving to end, add it now if not already present
                    elif not re.search(rf"[\[\({{\)]{re.escape(file.release_group)}[\]\)}}\)]", rename, re.IGNORECASE):
                ...
                if file.extras:
                    extras_to_add = [extra for extra in file.extras if not re.search(re.escape(extra.strip("()[]{} ")), rename, re.IGNORECASE)]
                    if extras_to_add:
                        rename += " " + " ".join(extras_to_add)

                # Add release group at the end if flagged
                if release_group_to_add and not re.search(rf"[\[\({{\)]{re.escape(release_group_to_add)}[\]\)}}\)]", rename, re.IGNORECASE):
                     rename += f" {modifiers.get(file.extension, '[%s]') % release_group_to_add}"

                # Final cleanup
                rename = rename.replace("*", "") # Remove potential wildcards if base_dir had them
                rename += file.extension
                rename = remove_dual_space(rename).strip() # Use imported string_utils function

                # Handle unicode and problematic characters
                if replace_unicode_when_restructuring and contains_unicode(rename): # Use imported string_utils function
                    rename = unidecode(rename) # Use imported unidecode
                rename = rename.replace('"', "'").replace("/", "-").replace(":", " - ") # Replace problematic chars
                ...
                if file.name != rename:
                    ...
                    if watchdog_toggle:
                        ...
                        if rename_path not in transferred_files: transferred_files.append(rename_path)
                        if file.path in transferred_files: transferred_files.remove(file.path)

                    try:
                        send_message(f"\n\t\tBEFORE: {file.name}", discord=False)
                        send_message(f"\t\tAFTER:  {rename}", discord=False)

                        user_input = get_input_from_user("\t\tReorganize & Rename", ["y", "n"], ["y", "n"]) if manual_rename else "y"

                        if user_input == "y":
                            if not os.path.isfile(rename_path):
                                rename_status = rename_file(file.path, rename_path, silent=True) # Use imported file_utils function

                                if not rename_status:
                                    send_message(f"Failed to rename {file.name} to {rename}", error=True)
                                    # Revert watchdog changes if rename failed
                                    if watchdog_toggle:
                                         if rename_path in transferred_files: transferred_files.remove(rename_path)
                                         if file.path not in transferred_files: transferred_files.append(file.path)
                                    continue # Skip to next file

                                send_message("\t\t\tSuccessfully reorganized & renamed file.\n", discord=False)

                                # Add to processed files list (used elsewhere)
                                if rename not in processed_files: processed_files.append(rename)

                                # Discord notification
                                if not mute_discord_rename_notifications:
                                    embed = handle_fields( # Use imported discord_utils function
                                        DiscordEmbed(title="Reorganized & Renamed File", color=grey_color), # Use imported discord_utils function
                                        fields=[
                                            {"name": "From", "value": f"```{file.name}```", "inline": False},
                                            {"name": "To", "value": f"```{rename}```", "inline": False},
                                            {"name": "Location", "value": f"```{file.root}```", "inline": False},
                                        ]
                                    )
                                    grouped_notifications = group_notification(grouped_notifications, Embed(embed, None)) # Use imported discord_utils and models
                                ...
                                try:
                                     ...
                                     new_volume_list = upgrade_to_volume_class(upgrade_to_file_class([rename], file.root))
                                     if new_volume_list:
                                         files[idx] = new_volume_list[0]
                                except (ValueError, IndexError):
                                     send_message(f"Could not find/update file object for {file.name} after rename.", error=True)


                            else:
                                # Target file already exists, delete source
                                print(f"\t\tFile already exists: {rename_path}. Deleting source: {file.name}")
                                remove_file(file.path, silent=True) # Use imported file_utils function
                            ...
                            if file.name not in processed_files: processed_files.append(file.name)
            ...
            send_message(f"Error in reorganize_and_rename for file {file.name}: {e}", error=True)
