...
try:
    from .config import (
        download_folders, sleep_timer, file_extensions, image_extensions,
        delete_unacceptable_files_toggle, convert_to_cbz_toggle, unacceptable_keywords,
        convertable_file_extensions, watchdog_discover_new_files_check_interval,
        watchdog_file_transferred_check_interval, purple_color, profile_code,
        compress_image_option, paths, # Added compress_image_option, paths
        transferred_files, transferred_dirs # Removed grouped_notifications
    )
except ImportError:
    print("WARN: Could not import from .config, using placeholder values in watchdog_handler.")
    download_folders = []
    sleep_timer = 10
...
try:
    from .file_utils import (
        get_file_extension, is_file_transferred,
        get_all_files_recursively_in_dir_watchdog, create_folder_obj
    )
except ImportError:
    print("WARN: Could not import from .file_utils, defining placeholders.")
    def get_file_extension(f): return os.path.splitext(f)[1]
    def is_file_transferred(f): return True # Assume transferred for placeholder
    def get_all_files_recursively_in_dir_watchdog(p): return []
    def create_folder_obj(p, **kwargs): return Folder(p, [], "", os.path.basename(p), []) # Needs Folder model

try:
    from .discord_utils import handle_fields, send_discord_message, group_notification, DiscordEmbed
except ImportError:
...
try:
    from .models import Embed, Folder # Import needed models
     ...
     class Embed: pass
     class Folder:
         def __init__(self, r, d, bn, fn, f): self.root=r # Basic placeholder
...
try:
    ...
    class core_logic:
        @staticmethod
        def main(): print("Placeholder core_logic.main() called.")
    ...
    def on_created(self, event):
        ...
        try:
            ...
            if src_path in self.run_transferred_files:
                print(f"\tWatchdog: File already processed in this run, skipping: {src_path}")
                return

            # Check file extension validity
            is_valid_convertable = extension in convertable_file_extensions # Use imported config value
            is_valid_direct = extension in file_extensions # Use imported config value

            if not is_valid_direct and not (convert_to_cbz_toggle and is_valid_convertable): # Use imported config value
                 if delete_unacceptable_files_toggle: # Use imported config value
                     # Check if it's explicitly unacceptable
                     is_unacceptable = any(re.search(kw, base_name, re.IGNORECASE) for kw in unacceptable_keywords) # Use imported config value
                     if is_unacceptable:
                          print(f"\tWatchdog: Unacceptable file detected: {src_path}. Will be handled by delete_unacceptable_files.")
            ...
            send_message("\nStarting Execution (WATCHDOG)", discord=False) # Use imported log_utils function
            ...
            while not is_file_transferred(src_path): # Use imported file_utils function
                ...
                if not os.path.isfile(src_path):
                    print(f"\tWatchdog: File disappeared while waiting for transfer: {src_path}")
                    return # Stop processing this event

            print(f"\tWatchdog: File transfer complete for: {src_path}")
            self.run_transferred_files.append(src_path)
            dir_path = os.path.dirname(src_path)
            if dir_path not in download_folders and dir_path not in self.run_transferred_dirs: # Use imported config value
            ...
            folder_objs_to_process = [create_folder_obj(d) for d in self.run_transferred_dirs] # Use imported file_utils function
            ...
            if profile_code == "core_logic.main()": # Use imported config value
                cProfile.runctx("core_logic.main(folders_to_process=folder_objs_to_process)", globals(), locals(), sort="cumtime")
            else:
            ...
            send_message(f"Error in Watchdog Handler on_created for {event.src_path}: {e}", error=True) # Use imported log_utils function
            import traceback
            ...
            send_message(f"\nFinished Execution (WATCHDOG)\n\tExecution Time: {exec_time_str}", discord=False) # Use imported log_utils function
            # Discord notification responsibility moved
...
class Watcher:
    ...
    def __init__(self):
        self.observers = []
        self.lock = threading.Lock() # Lock for potential shared resources if Handler logic needs it
    ...
    def run(self):
        ...
        for folder in download_folders:
            if not os.path.isdir(folder):
                print(f"Watcher: Configured download folder does not exist or is not a directory: {folder}")
                continue
        ...
        try:
            while True:
                ...
                time.sleep(sleep_timer) # Use imported config value (or a different interval for main thread)
    ...
    def stop(self):
        ...
        for observer in self.observers:
            if observer.is_alive():
                observer.stop()
                print(f"Watcher: Observer stopped for {observer.name}") # Observer might not have name attr always
