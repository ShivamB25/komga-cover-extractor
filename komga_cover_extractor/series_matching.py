...
try:
    ...
    print(
        "WARN: Could not import from .config, using placeholder values in series_matching."
    )
    paths = []
    download_folders = []
    paths_with_types = []
...
try:
    ...
    from .file_utils import (
        ...
        sort_volumes,  # Added sort_volumes
    )
    ...
    from .archive_utils import (
        ...
        get_identifiers as get_ids_from_comment,
    )
    ...
    def upgrade_to_volume_class(*args, **kwargs):
        return []
    ...
    def get_subtitle_from_dash(*args, **kwargs):
        return ""
    ...
    def find_consecutive_items(*args, **kwargs):
        return False

    def count_words(l):
        return {}

    def move_strings_to_top(t, a):
        return a

    def array_to_string(a, **kwargs):
        return str(a)

    def abbreviate_numbers(n):
        return str(n)

    def complete_num_array(a):
        return a

    def is_same_index_number(*args, **kwargs):
        return False

    def reorganize_and_rename(f, d):
        return f

    def find_and_extract_cover(*args, **kwargs):
        return None

    def prep_images_for_similarity(*args, **kwargs):
        return 0.0

    def get_zip_comment(p):
        return ""

    def get_zip_comment_cache(p):
        return ""

    def get_ids_from_comment(c):
        return []
...
def is_upgradeable(downloaded_release, current_release):
    ...
    if downloaded_release.name == current_release.name:
        ...
        results = get_keyword_scores(
            [downloaded_release]
        )  # Use local placeholder/imported function
...
def remove_duplicate_releases(
    ...
    def get_file_tags_info(result):
        ...
        if tags:
            ...
            return ", ".join(
                [
                    f"{getattr(tag, 'name', '?')} ({getattr(tag, 'score', '?')})"
                    for tag in tags
                ]
            )
        return "None"
    ...
    def get_file_size_info(path):
        ...
        return (
            f"{round(size_bytes / 1000000, 1)} MB" if size_bytes is not None else None
        )
    ...
    for download in downloaded_releases:
        ...
        try:
            all_indices = [
                item.index_number
                for item in new_original_releases + new_downloaded_releases
                if item.index_number != ""
            ]
            hashable_indices = [
                tuple(idx) if isinstance(idx, list) else idx for idx in all_indices
            ]
        ...
        for original in original_releases:
            if original not in new_original_releases or not os.path.isfile(
                original.path
            ):
                continue
            if not os.path.isfile(download.path):
                break  # Download removed previously

            if download.file_type != original.file_type:
                continue

            indices_match = is_same_index_number(
            ...
            if not (indices_match or image_match_cond):
                continue

            try:
                upgrade_status = is_upgradeable(
                    download, original
                )  # Use local function
            except Exception as upgrade_err:
                send_message(
                    f"Error checking upgrade status for {download.name} vs {original.name}: {upgrade_err}",
                    error=True,
                )
                continue
            ...
            if original_file_size:
                fields.append(
                    {"name": "Size", "value": str(original_file_size), "inline": True}
                )
            fields.extend(
                [
            ...
            if downloaded_file_size:
                fields.append(
                    {"name": "Size", "value": str(downloaded_file_size), "inline": True}
                )
            ...
            try:
                embed = handle_fields(
                    DiscordEmbed(title=f"Upgrade Process ({status})", color=color),
                    fields=fields,
                )  # Use imported discord_utils
                grouped_notifications = group_notification(
                    grouped_notifications, Embed(embed, None)
                )  # Use imported discord_utils
            except Exception as discord_err:
                send_message(
                    f"Error creating/grouping Discord embed for upgrade check: {discord_err}",
                    error=True,
                )
            ...
            if upgrade_status.is_upgrade:
                ...
                if (
                    ...
                    files_to_remove = [
                        orig_vol
                        for orig_vol in new_original_releases
                    ...
                    for file_to_remove in files_to_remove:
                        if remove_file(file_to_remove.path):  # Use imported file_utils
                            if file_to_remove in new_original_releases:
                                new_original_releases.remove(file_to_remove)
                ...
                print(
                    "WARN: replace_file function not implemented yet in series_matching."
                )
                ...
                if replace_file_status:
                    ...
                    if original in new_original_releases:
                        new_original_releases.remove(original)
                    break
                else:
                    send_message(
                        f"Upgrade failed for {download.name} -> {original.name}. Check logs.",
                        error=True,
                    )
                    break
                ...
                user_input = (
                    get_input_from_user(
                        f'\t\t\tDelete "{download.name}"', ["y", "n"], ["y", "n"]
                    )
                    if manual_delete
                    else "y"
                )  # Use imported misc_utils
                if user_input == "y":
                    if remove_file(download.path):  # Use imported file_utils
                        ...
                        send_message(
                            f"Failed to delete non-upgrade file: {download.path}",
                            error=True,
                        )
                        break
                else:
...
def check_for_existing_series(
    ...
    def group_similar_series(messages):
        ...
        return [
            {"series_name": series, "messages": msgs}
            for series, msgs in grouped.items()
        ]
    ...
    def alternative_match_allowed(
        ...
        folder_subtitle = get_subtitle_from_dash(
            inner_dir, replace=True
        )  # Use imported string_utils
        folder_subtitle_clean = (
            clean_str(folder_subtitle) if folder_subtitle else ""
        )  # Use imported string_utils

        file_subtitle = get_subtitle_from_dash(
            file.series_name, replace=True
        )  # Use imported string_utils
        file_subtitle_clean = (
            clean_str(file_subtitle) if file_subtitle else ""
        )  # Use imported string_utils

        short_fldr_name = clean_str(
        ...
        if not short_fldr_name_words or not short_file_series_words:
            return False
        ...
        shortened_length = max(
            1,
            int(
                min(len(short_fldr_name_words), len(short_file_series_words))
                * short_word_filter_percentage
            ),
        )
        ...
        consecutive_items_match = find_consecutive_items(
            ...
            tuple(long_folder_words), tuple(long_file_words)
        )  # Use imported string_utils
        unique_words_match = any(
            i in long_file_words and i in counted_words and counted_words[i] <= 3
    ...
    def attempt_alternative_match(
        ...
        try:
            ...
            matching_volumes = [
                vol
                for vol in img_volumes
                if is_same_index_number(
                    vol.index_number, file.index_number, allow_array_match=True
                )
            ]  # Use imported string_utils
            ...
            downloaded_cover_data = find_and_extract_cover(
                file, return_data_only=True, silent=True, blank_image_check=True
            )  # Use imported image_utils
            if not downloaded_cover_data:
                return 0, None

            for matching_volume in matching_volumes:
                print(f"\t\t\tComparing covers: {matching_volume.name} vs {file.name}")
                existing_cover_data = find_and_extract_cover(
                    matching_volume,
                    return_data_only=True,
                    silent=True,
                    blank_image_check=True,
                )  # Use imported image_utils
                if not existing_cover_data:
                    continue

                score = prep_images_for_similarity(
                    existing_cover_data,
                    downloaded_cover_data,
                    both_cover_data=True,
                    silent=True,
                )  # Use imported image_utils
                print(
                    f"\t\t\t\tCover Image Similarity Score: {score} (Required: {required_image_similarity_score})"
                )
                ...
                if score >= required_image_similarity_score:
                    return (
                        score,
                        matching_volume,
                    )  # Return score and the specific volume matched

            return 0, None  # No match found
        except Exception as e:
            send_message(
                f"Error during alternative match for {file.name} in {file_root}: {e}",
                error=True,
            )
            return 0, None
    ...
    def check_upgrade(
        ...
        if not os.path.isdir(existing_dir):
            send_message(
                f"Target directory {existing_dir} not found during upgrade check.",
                error=True,
            )
        ...
        try:
            existing_files_names = [
                entry.name for entry in scandir.scandir(existing_dir) if entry.is_file()
            ]
            clean_existing_objs = upgrade_to_volume_class(  # Use imported file_utils
                upgrade_to_file_class(
                    existing_files_names, existing_dir, clean=True
                ),  # Use imported file_utils
            ...
            send_message(
                f"Error reading/processing existing files in {existing_dir}: {e}",
                error=True,
            )
            return False
        ...
        if rename_files_in_download_folders_toggle and resturcture_when_renaming:
            ...
            try:
                download_dir_volumes = reorganize_and_rename(
                    download_dir_volumes, existing_dir
                )  # Use imported string_utils
            except Exception as rename_err:
                send_message(
                    f"Error during pre-upgrade rename for {file.name}: {rename_err}",
                    error=True,
                )
        ...
        title = (
            "Found Series Match"
            + (" (CACHE)" if cache else "")
            + (" (Identifier)" if isbn else "")
            + (" (Cover Match)" if image else "")
        )
        try:
            embed = handle_fields(
                DiscordEmbed(title=title, color=grey_color), fields=fields
            )  # Use imported discord_utils
            grouped_notifications = group_notification(
                grouped_notifications, Embed(embed, None)
            )  # Use imported discord_utils
            ...
            send_message(
                f"Error creating/grouping Discord embed for series match: {discord_err}",
                error=True,
            )

        # --- Remove Duplicates/Perform Upgrade ---
        if test_mode:
            return clean_existing_objs  # Return existing for testing

        clean_existing_objs, download_dir_volumes = (
            remove_duplicate_releases(  # Use local function
        ...
        if (
        ...
        ):  # If the downloaded volume wasn't removed as a duplicate
            volume = download_dir_volumes[0]
            if isinstance(
                volume.volume_number, (float, int, list)
                ...
                if volume.file_type == "volume" or (
                    ...
                    cover_data = find_and_extract_cover(
                        volume, return_data_only=True, silent=True
                    )  # Use imported image_utils
                ...
                try:
                    all_indices = [
                        item.index_number
                        for item in clean_existing_objs + download_dir_volumes
                        if item.index_number != ""
                    ]
                    hashable_indices = [
                ...
                move_status = move_file(
                    volume, existing_dir, highest_index_num=highest_index_num
                )  # Use imported file_utils
                ...
                if move_status:
                    ...
                    if new_file_path not in moved_files:
                        moved_files.append(new_file_path)  # Manage global state

                    # Send Discord notification for new release
                    try:
                        embed = handle_fields(
                            DiscordEmbed(
                                title=f"New {release_type}(s) Added", color=green_color
                            ),
                            fields=new_release_fields,
                        )  # Use imported discord_utils
                        if new_volume_webhook:
                            ...
                            if volume.file_type == "chapter":
                                ...
                                send_discord_message(
                                    None,
                                    [Embed(embed, cover_data)],
                                    passed_webhook=new_volume_webhook,
                                )  # Use imported discord_utils
                        else:
                            grouped_notifications = group_notification(
                                grouped_notifications, Embed(embed, cover_data)
                            )  # Use imported discord_utils
                    except Exception as discord_err:
                        send_message(
                            f"Error creating/sending Discord embed for new release: {discord_err}",
                            error=True,
                        )
                    ...
                    send_message(
                        f"Failed to move new file {volume.name} to {existing_dir}",
                        error=True,
                    )
            ...
            check_and_delete_empty_folder(
                file.root
            )  # Check original download folder if empty
    ...
    for download_folder in download_folders_local:
        ...
        if isinstance(download_folder, Folder):  # If watchdog passed Folder objects
            folders_to_scan = [
                {
                    "root": download_folder.root,
                    "dirs": download_folder.dirs,
                    "files": [f.name for f in download_folder.files],
                }
            ]
            ...
            try:
                ...
                folders_to_scan = [
                    {"root": download_folder, "dirs": top_dirs, "files": top_files}
                ]
                # Add subdirectories if needed, or rely on watchdog passing specific folders
                folders_to_scan.extend(
                    [
                        {
                            "root": os.path.join(download_folder, d),
                            "dirs": [],
                            "files": [],
                        }
                        for d in top_dirs
                    ]
                )  # Basic structure for subdirs
            except Exception as e:
                send_message(
                    f"Error scanning download folder {download_folder}: {e}", error=True
                )
                continue
        ...
        for folder_info in folders_to_scan:
            ...
            if (
                ...
                and not folder_info["files"]
                and not folder_info["dirs"]
            ):
                try:
            ...
            for file in volumes:
                try:
                    if not file.series_name or file.volume_number == "":
                        continue
                    if not (test_mode or os.path.isfile(file.path)):
                        continue  # Ensure file exists
                    ...
                    if (
                        series_key in unmatched_series
                        and not match_through_identifiers
                        and not match_through_image_similarity
                    ):
                    ...
                    if (
                        ...
                        # Check cache first
                        cached_id_match = next(
                            (
                                ci
                                for ci in cached_identifier_results
                                if ci.series_name == file.series_name
                            ),
                            None,
                        )
                        if cached_id_match:
                            done = check_upgrade(
                                os.path.dirname(cached_id_match.path),
                                os.path.basename(cached_id_match.path),
                                file,
                                similarity_strings=cached_id_match.matches,
                                isbn=True,
                            )
                            if done and cached_id_match.path not in cached_paths:
                                cache_path(
                                    cached_id_match.path
                                )  # Use imported file_utils
                            if done:
                        ...
                        dl_zip_comment = (
                            get_zip_comment(file.path) if not test_mode else ""
                        )  # Use imported archive_utils
                        dl_meta_ids = (
                            get_ids_from_comment(dl_zip_comment)
                            if dl_zip_comment
                            else []
                        )  # Use imported archive_utils
                    ...
                    if not done:
                        ...
                        if cached_paths_local:
                            ...
                            current_cached_paths = cached_paths_local[
                                :
                            ]  # Work on a copy
                            current_cached_paths = move_strings_to_top(
                                file.series_name, current_cached_paths
                            )  # Use imported string_utils
                            current_cached_paths = move_strings_to_top(
                                downloaded_file_series_name, current_cached_paths
                            )  # Use imported string_utils

                            for p in current_cached_paths:
                                if not os.path.isdir(p) or p in download_folders_local:
                                    continue
                                ...
                                successful_similarity_score = similar(
                                    successful_series_name, downloaded_file_series_name
                                )  # Use imported string_utils
                                ...
                                if (
                                    ...
                                    if done:
                                        if p not in cached_paths:
                                            cache_path(p)  # Use imported file_utils
                        ...
                        if not done and paths_local:
                            counted_words_global = (
                                {}
                            )  # Cache word counts across library?
                            for path_root in paths_local:
                                if done:
                                    break
                                if (
                                    not os.path.isdir(path_root)
                                    or path_root in download_folders_local
                                ):
                                    continue
                                # Path filtering based on type...
                                try:
                                    # Walk through library path
                                    for lib_root, lib_dirs, _ in scandir.walk(
                                        path_root
                                    ):
                                        if done:
                                            break
                                        ...
                                        # Reorganize lib_dirs...
                                        lib_dirs = move_strings_to_top(
                                            file.series_name, lib_dirs
                                        )  # Use imported string_utils

                                        for inner_dir in lib_dirs:
                                            existing_series_folder = clean_str(
                                                inner_dir
                                            )  # Use imported string_utils
                                            similarity_score = similar(
                                                existing_series_folder,
                                                downloaded_file_series_name,
                                            )  # Use imported string_utils
                                            ...
                                            if (
                                                ...
                                                if done:
                                                    ...
                                                    if match_path not in cached_paths:
                                                        cache_path(
                                                            match_path
                                                        )  # Use imported file_utils
                                                    # Cache reordering...
                                            ...
                                            elif alternative_match_allowed(
                                                inner_dir,
                                                file,
                                                short_word_filter_percentage,
                                                required_similarity_score,
                                    ...
                                    send_message(
                                        f"Error walking library path {path_root}: {walk_err}",
                                        error=True,
                                    )
                    ...
                    send_message(
                        f"Error processing file {file.name}: {file_err}\n{traceback.format_exc()}",
                        error=True,
                    )
        ...
        if messages_to_send:
            grouped_by_series = group_similar_series(
                messages_to_send
            )  # Use local helper
            ...
            webhook_to_use = (
                new_volume_webhook  # Use the specific chapter webhook if set
            )
            ...
            for group in grouped_by_series:
                ...
                embed = handle_fields(
                    DiscordEmbed(title=first_item.title, color=first_item.color),
            ...
            if series_notifications and webhook_to_use:
                send_discord_message(
                    None, series_notifications, passed_webhook=webhook_to_use
                )  # Use imported discord_utils
        ...
        for p in paths_to_search:
            ...
            for root, dirs, files in scandir.walk(p):
                ...
                for key, group in potential_duplicates.items():
                    if len(group) > 1:
                        print(
                            f"\t\tFound {len(group)} potential duplicates for index {key[0]} in {key[1]}"
                        )
                        ...
                        try:
                            ...
                            send_message(
                                f"Error sorting duplicates for {key}: {sort_err}",
                                error=True,
                            )
    ...
    def check_upgrade(
        ...
        if rename_files_in_download_folders_toggle and resturcture_when_renaming:
            ...
            try:
                ...
                download_dir_volumes = reorg_rename_op(
                    download_dir_volumes, existing_dir
                )
            except ImportError:
                send_message(
                    "Could not import reorganize_and_rename from file_operations for pre-upgrade rename.",
                    error=True,
                )
            except Exception as rename_err:
                send_message(
        ...
        if similarity_strings:
            if not isbn and not image:
                ...
                fields.extend(
                    [
                        {
                            "name": "Downloaded File Identifiers",
                            "value": "```" + "\n".join(similarity_strings[0]) + "```",
                            "inline": False,
                        },
                        {
                            "name": "Existing Library File Identifiers",
        ...
        clean_existing_objs, download_dir_volumes = (
            remove_duplicate_releases(  # Use function defined in this module
                clean_existing_objs, download_dir_volumes, image_similarity_match=image
            )
        )
    ...
    for download_folder in download_folders_local:
        ...
        for folder_info in folders_to_scan:
            ...
            for file in volumes:
                try:
                    ...
                    if match_through_image_similarity and not done:
                        # Check cache first
                        cached_img_match = next(
                            (
                                ci
                                for ci in cached_image_similarity_results
                                if f"{file.series_name} - {file.file_type} - {file.root} - {file.extension}"
                                in ci
                            ),
                            None,
                        )
        ...
        if messages_to_send:
            ...
            for group in grouped_by_series:
                ...
                embed = handle_fields(
                    DiscordEmbed(title=embed_title, color=embed_color),
                    fields=combined_fields,
                )  # Use imported discord_utils
