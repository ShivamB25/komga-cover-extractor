...
try:
    ...
    print(
        "WARN: Could not import from .config in archive_utils.py, using placeholders."
    )
...
def extract(file_path, temp_dir, extension):
    ...
    try:
        if extension in rar_extensions:  # Use imported config value
            ...
            send_message(
                f"Unsupported extension for extraction: {extension}", error=True
            )  # Use imported log_utils function
    except rarfile.NeedFirstVolume:
        send_message(
            f"Extraction failed: Multi-volume RAR detected, cannot process {file_path}",
            error=True,
        )
        ...
        if extension in rar_extensions and "unrar" in str(fnf_error):
            send_message(
                f"Extraction failed: 'unrar' command not found or not executable. Please install it or check PATH.",
                error=True,
            )
        else:
            send_message(
                f"Extraction failed: File not found during extraction {file_path}: {fnf_error}",
                error=True,
            )
    except Exception as e:
        send_message(
            f"Error extracting {file_path}: {e}", error=True
        )  # Use imported log_utils function
    return success
...
def compress(temp_dir, cbz_filename):
    ...
    try:
        ...
        send_message(
            f"Error compressing {temp_dir} to {cbz_filename}: {e}", error=True
        )  # Use imported log_utils function
            ...
            for entry in files:
                try:
                    ...
                    if (
                        ...
                        if (
                            manual_rename
                        ):  # Use imported config value (assuming this flag controls conversion prompt too)
                            user_input = get_input_from_user(
                                "\t\t\tConvert to CBZ?", ["y", "n"], ["y", "n"]
                        ...
                        if remove_file(
                            ...
                            send_message(
                                f"\t\t\tFailed to remove original file {source_file}",
                                error=True,
                            )  # Use imported log_utils function

                except Exception as e:
                    send_message(
                        f"Error during conversion process for {entry}: {e}", error=True
                    )  # Use imported log_utils function
...
def get_zip_comment(zip_file):
    ...
    try:
        with zipfile.ZipFile(zip_file, "r") as zip_ref:
            if zip_ref.comment:
                ...
                try:
                    ...
                    try:
                        comment = zip_ref.comment.decode(
                            "cp437"
                        )  # Try common fallback encoding
                    except Exception:
                        comment = str(
                            zip_ref.comment
        ...
        send_message(
            f"Bad zip file, cannot read comment: {zip_file}", error=True
        )  # Use imported log_utils function
    except FileNotFoundError:
        send_message(
            f"File not found, cannot read comment: {zip_file}", error=True
        )  # Use imported log_utils function
    except Exception as e:
        send_message(
            f"Error reading zip comment for {zip_file}: {e}", error=True
        )  # Use imported log_utils function
    return comment
...
def contains_comic_info(zip_file):
    ...
    try:
        ...
        send_message(
            f"Cannot check for ComicInfo.xml in {zip_file}: {e}", error=True
        )  # Use imported log_utils function
    except Exception as e:  # Catch other potential zip errors
        send_message(
            f"Error accessing zip {zip_file} for ComicInfo check: {e}", error=True
        )
    return result
    ...
    try:
        with zipfile.ZipFile(zip_file_path, "r") as z:
            ...
            for path in file_list:
                ...
                name_to_check = (
                    os.path.basename(path).lower() if allow_base else path.lower()
                )

                # Check if any search pattern matches
                if any(
                    re.search(search_pattern, name_to_check, re.IGNORECASE)
                    for search_pattern in lower_searches
                ):
                    result = z.read(path)
                    break  # Found the first match
    except (zipfile.BadZipFile, FileNotFoundError) as e:
        send_message(
            f"Cannot read from zip {zip_file_path}: {e}", error=True
        )  # Use imported log_utils function
    except Exception as e:
