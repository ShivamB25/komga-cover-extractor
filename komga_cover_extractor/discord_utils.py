...
try:
    ...
    print(
        "WARN: Could not import from .config in discord_utils.py, using placeholders."
    )
    discord_webhook_url = []
    script_version_text = ""
...
@lru_cache(
    maxsize=10
)  # Cache based on passed_webhook/url to avoid re-calculating index unnecessarily
    ...
    try:
        ...
        if embeds:
            ...
            for index, embed_wrapper in enumerate(embeds_to_send):
                ...
                if not isinstance(embed_wrapper, Embed) or not isinstance(
                    embed_wrapper.embed, DiscordEmbed
                ):
                    send_message(
                        f"Invalid embed object type in list: {type(embed_wrapper)}",
                        error=True,
                    )
                    continue
                ...
                if (
                    timestamp and not discord_embed.timestamp
                ):  # Check if timestamp already set
                ...
                if embed_wrapper.file:
                    try:
                        ...
                        webhook_obj.add_file(
                            file=embed_wrapper.file, filename=file_name
                        )
                        # Set image URL to attachment
                        discord_embed.set_image(url=f"attachment://{file_name}")
                    except Exception as file_err:
                        send_message(
                            f"Error adding file to Discord message: {file_err}",
                            error=True,
                        )
        ...
        send_message(
            f"Error sending Discord message to {hook}: {e}", error=True, discord=False
        )  # Avoid recursion
        sent_status = False
    finally:
...
def group_notification(notifications_list, embed_wrapper, passed_webhook=None):
    ...
    if not isinstance(embed_wrapper, Embed):
        send_message(
            f"Invalid object passed to group_notification: {type(embed_wrapper)}",
            error=True,
        )
    ...
    if (
        isinstance(embed_wrapper.embed, DiscordEmbed)
        and not embed_wrapper.embed.timestamp
    ):
    ...
    if len(notifications_list) >= discord_embed_limit:  # Use imported config value
        send_discord_message(
            None, embeds=notifications_list, passed_webhook=passed_webhook
        )  # Use local function
