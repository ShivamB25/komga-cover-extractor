...
try:
    ...
    from komga_cover_extractor.file_utils import (
        get_all_files_in_directory,
        get_file_extension,
    )
    from komga_cover_extractor.string_utils import (
        contains_volume_keywords,
        contains_chapter_keywords,
    )
    from komga_cover_extractor.models import (
        Path,
        LibraryType,
    )  # Path needed for process_path
...
def process_path(
    path_arg_list, paths_with_types_list, paths_list, is_download_folders=False
):
    ...
    def process_auto_classification():
        ...
        try:
            ...
            files_to_check = (
                files[: int(len(files) * 0.75)] if len(files) >= 100 else files
            )
            print(f"\t\t\t\t- checking {len(files_to_check)} files.")

            # Extensions
            all_extensions = [
                get_file_extension(file) for file in files_to_check
            ]  # Use imported function
            common_exts = get_common_extensions(all_extensions)
            path_extensions.extend(
                ext for ext in common_exts if ext not in path_extensions
            )
            ...
            for ext_set in [config.manga_extensions, config.novel_extensions]:
                if any(ext in path_extensions for ext in ext_set):
                    path_extensions.extend(
                        e for e in ext_set if e not in path_extensions
                    )
            if not path_extensions:
                path_extensions.extend(config.file_extensions)  # Default if none found
            ...
            all_types = [
                (
                    "chapter"
                    if (
                        not contains_volume_keywords(f) and contains_chapter_keywords(f)
                    )
                    else "volume"
            ...
            if total_files > 0:
                if chapter_count / total_files >= CHAPTER_THRESHOLD:
                    path_formats.append("chapter")
                elif volume_count / total_files >= VOLUME_THRESHOLD:
                    path_formats.append("volume")
            if not path_formats:
                path_formats.extend(config.file_formats)  # Default
            print(f"\t\t\t\t\t- path formats: {path_formats}")

        except Exception as auto_err:
            send_message(
                f"Error during auto-classification for {path_str}: {auto_err}",
                error=True,
            )  # Use imported function
    ...
    def get_common_extensions(all_extensions):
        ...
        return [
            ...
            if count / len(all_extensions) >= COMMON_EXTENSION_THRESHOLD
        ]
    ...
    def process_single_type_path(path_to_process):
        ...
        if first_part in config.file_formats:
            path_formats.extend(
                p for p in parts if p in config.file_formats and p not in path_formats
            )
        elif first_part.startswith(".") and first_part in config.file_extensions:
            path_extensions.extend(
                p
                for p in parts
                if p in config.file_extensions and p not in path_extensions
            )
        elif first_part in [lt.name for lt in config.library_types]:
            path_library_types.extend(
                p
                for p in parts
                if p in [lt.name for lt in config.library_types]
                and p not in path_library_types
            )
        elif first_part in config.translation_source_types:
            path_translation_source_types.extend(
                p
                for p in parts
                if p in config.translation_source_types
                and p not in path_translation_source_types
            )
    ...
    if not os.path.isdir(path_str):
        send_message(
            f"Warning: Path does not exist or is not a directory: {path_str}",
            error=True,
        )  # Use imported function
    ...
    if len(path_arg_list) == 1:  # No type hints provided
        ...
        if (
            not is_download_folders
            and config.watchdog_toggle
            and config.auto_classify_watchdog_paths
            and config.check_for_existing_series_toggle
        ):
        ...
        path_obj = Path(
            ...
            library_types=[
                lt for lt in config.library_types if lt.name in path_library_types
            ]
            or config.library_types,  # Convert names back to objects
            translation_source_types=path_translation_source_types
            or config.translation_source_types,
    ...
    if not is_download_folders:
        if path_str not in config.paths:
            config.paths.append(path_str)
        if path_obj and path_obj not in config.paths_with_types:
            config.paths_with_types.append(path_obj)
    else:
        if path_str not in config.download_folders:
            config.download_folders.append(path_str)
        if path_obj and path_obj not in config.download_folders_with_types:
            config.download_folders_with_types.append(path_obj)
...
def parse_my_args():
    ...
    parser = argparse.ArgumentParser(
        description=f"Komga Cover Extractor {config.script_version_text}"
    )
    ...
    parser.add_argument(
        "-wh",
        "--webhook",
        help="Discord webhook URL(s)",
        action="append",
        nargs="*",
        required=False,
    )
    parser.add_argument(
        "-bwc", "--bookwalker_check", help="Enable Bookwalker check", required=False
    )
    parser.add_argument(
        "-c", "--compress", help="Enable image compression", required=False
    )
    parser.add_argument(
        "-cq", "--compress_quality", help="Image compression quality", required=False
    )
    parser.add_argument(
        "-bwk_whs",
        "--bookwalker_webhook_urls",
        help="Bookwalker webhook URLs",
        action="append",
        nargs="*",
        required=False,
    )
    parser.add_argument(
        "-wd", "--watchdog", help="Enable watchdog mode", required=False
    )
    parser.add_argument(
        "-nw",
        "--new_volume_webhook",
        help="Specific webhook for new volumes",
        required=False,
    )
    parser.add_argument(
        "-ltf", "--log_to_file", help="Enable logging to file", required=False
    )
    parser.add_argument(
        "--watchdog_discover_new_files_check_interval",
        help="Watchdog discovery interval",
        required=False,
    )
    parser.add_argument(
        "--watchdog_file_transferred_check_interval",
        help="Watchdog transfer check interval",
        required=False,
    )
    parser.add_argument(
        "--output_covers_as_webp", help="Output covers as WebP", required=False
    )
    ...
    if args.download_folders:
        ...
        for df_item in processed_df:
            ...
            process_path(
                path_parts,
                config.download_folders_with_types,
    ...
    if args.watchdog:
        if config.download_folders:
            ...
            send_message(
                "Watchdog requires download folders to be specified.", error=True
            )
        ...
        for item in args.webhook:
            if item:
                for hook in item:
                    if hook:
                        if r"\1" in hook:
                            urls.extend(h for h in hook.split(r"\1") if h)
                        else:
                            urls.append(hook)
        ...
        for item in args.bookwalker_webhook_urls:
            if item:
                for hook in item:
                    if hook:
                        if r"\1" in hook:
                            bw_urls.extend(h for h in hook.split(r"\1") if h)
                        else:
                            bw_urls.append(hook)
    ...
    if (
        args.watchdog_discover_new_files_check_interval
        and args.watchdog_discover_new_files_check_interval.isdigit()
    ):
        config.watchdog_discover_new_files_check_interval = int(
            args.watchdog_discover_new_files_check_interval
        )
    if (
        args.watchdog_file_transferred_check_interval
        and args.watchdog_file_transferred_check_interval.isdigit()
    ):
        config.watchdog_file_transferred_check_interval = int(
            args.watchdog_file_transferred_check_interval
        )

    if config.watchdog_toggle:
        print(
            f"\t\twatchdog_discover_new_files_check_interval: {config.watchdog_discover_new_files_check_interval}"
        )
        print(
            f"\t\twatchdog_file_transferred_check_interval: {config.watchdog_file_transferred_check_interval}"
        )
    ...
    try:
        ...
        for setting_name in settings_vars:
            if setting_name in ignored_settings:
                continue
            value = getattr(settings_file, setting_name)
            display_value = (
                "********"
                if value
                and any(kw in setting_name.lower() for kw in sensitive_keywords)
                else value
            )
    ...
    if config.send_scan_request_to_komga_libraries_toggle and (
        config.check_for_existing_series_toggle
        or config.move_series_to_correct_library_toggle
    ):
...
def check_required_settings():
    ...
    try:
        ...
        print(
            f"Error checking required settings: {e}"
        )  # Avoid exiting if settings.py itself is missing
